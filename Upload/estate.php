...
if (isset($_GET['property']) && is_numeric($_GET['property'])) {
	...
	$npq              = $db->query("SELECT `hWILL`, `hPRICE`, `hNAME`
    				 FROM `houses`
	...
	if ($np['hWILL'] < $mp['hWILL']) {
		...
		$db->query("UPDATE `users`
                 ...
                 WHERE `userid` = $userid");
		echo "Congrats, you bought the {$np['hNAME']} for " . money_formatter($np['hPRICE']) . "!";
	}
...
The houses you can buy are listed below. Click a house to buy it.<br />";
	...
	while ($r = $db->fetch_row($hq)) {
		echo "<a href='estate.php?property={$r['hID']}'>{$r['hNAME']}</a>" . "&nbsp;&nbsp - Cost: " . money_formatter($r['hPRICE']) . "&nbsp;&nbsp - Will Bar: {$r['hWILL']}<br />";
	}
	$db->free_result($hq);
}
$h->endpage();
