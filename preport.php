...
$_POST['report'] = (isset($_POST['report']) && is_string($_POST['report'])) ? $db->escape(strip_tags(stripslashes($_POST['report']))) : '';
$_POST['player'] = (isset($_POST['player']) && is_numeric($_POST['player'])) ? abs(intval($_POST['player'])) : '';
if ($_POST['report'] && $_POST['player']) {
	if (strlen($_POST['report']) > 500) {
		echo 'You may only enter 500 characters or less here.
        <br />&gt;<a href="preport.php?ID=' . $_GET['player'] . '">Go Back</a>';
		die($h->endpage());
	}
	if (!isset($_POST['verf']) || !verify_csrf_code('preport_send', stripslashes($_POST['verf']))) {
		echo '<h3>Error</h3><hr />
	...
	$db->free_result($q);
	$db->query("INSERT INTO `preports`
             VALUES(NULL, $userid, {$_POST['player']}, '{$_POST['report']}')");
	echo 'Report processed!<br />
    &gt; <a href="index.php">Home</a>';
} else {
	$_GET['report'] = (isset($_GET['report']) && is_string($_GET['report'])) ? htmlentities(strip_tags(stripslashes($_GET['report'])), ENT_QUOTES, 'ISO-8859-1') : '';
