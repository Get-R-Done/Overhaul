...
$_POST['location'] = (isset($_POST['location']) && is_numeric($_POST['location'])) ? abs(intval($_POST['location'])) : '';
if (!$_POST['location']) {
	echo 'Invalid use of file';
} else {
	$check_it = $db->query('SELECT `cityid`
                     FROM `cities`
	...
	$db->free_result($check_it);
	$q = $db->query("SELECT `userid`, `level`, `money`, `crystals`, `username`
                     FROM `users`
