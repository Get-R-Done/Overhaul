...
<br /><br />";
$q   = $db->query("SELECT `donatordays`, `username`, `userid`, `money`, `level`,
...
while ($r = $db->fetch_row($q)) {
	$r['username'] = ($r['donatordays']) ? '<span style="color:red; font-weight:bold;">' . $r['username'] . '</span> <img src="donator.gif" alt="Donator: ' . $r['donatordays'] . ' Days Left" title="Donator: ' . $r['donatordays'] . ' Days Left" />' : $r['username'];
	echo '
		<tr>
			<td>' . $r['userid'] . '</td>
			<td><a href="viewuser.php?u=' . $r['userid'] . '">' . $r['gangPREF'] . ' ' . $r['username'] . '</a></td>
			<td>' . money_formatter($r['money']) . '</td>
			<td>' . $r['level'] . '</td>
			<td>' . $r['gender'] . '</td>
			<td>' . (($r['laston'] >= $_SERVER['REQUEST_TIME'] - 15 * 60) ? '<span style="color: green; font-weight:bold;">Online</span>' : '<span style="color: red; font-weight:bold;">Offline</span>') . '
			</td>
		</tr>
   ';
}
