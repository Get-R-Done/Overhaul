...
function addcourse()
{
	...
	$energy        = (isset($_POST['energy']) && is_numeric($_POST['energy'])) ? abs(intval($_POST['energy'])) : '';
	$days          = (isset($_POST['days']) && is_numeric($_POST['days'])) ? abs(intval($_POST['days'])) : '';
	$str           = (isset($_POST['str']) && is_numeric($_POST['str'])) ? abs(intval($_POST['str'])) : '';
	$agil          = (isset($_POST['agil']) && is_numeric($_POST['agil'])) ? abs(intval($_POST['agil'])) : '';
	$gua           = (isset($_POST['gua']) && is_numeric($_POST['gua'])) ? abs(intval($_POST['gua'])) : '';
	$lab           = (isset($_POST['lab']) && is_numeric($_POST['lab'])) ? abs(intval($_POST['lab'])) : '';
	...
	$_POST['desc'] = (isset($_POST['desc']) && preg_match("/^[a-z0-9_.]+([\\s]{1}[a-z0-9_.]|[a-z0-9_.])+$/i", $_POST['desc'])) ? $db->escape(strip_tags(stripslashes($_POST['desc']))) : '';
	if ($_POST['name'] && $_POST['desc'] && $cost && $days && $cost && $energy && $str && $agil && $gua && $lab && $iq) {
		staff_csrf_stdverify('staff_addcourse', 'staff_courses.php?action=addcourse');
...
function editcourse()
{
	...
	switch ($_POST['step']) {
		case "2":
			...
			$_POST['name'] = (isset($_POST['name']) && preg_match("/^[a-z0-9_]+([\\s]{1}[a-z0-9_]|[a-z0-9_])+$/i", $_POST['name'])) ? $db->escape(strip_tags(stripslashes($_POST['name']))) : '';
			$_POST['desc'] = (isset($_POST['desc']) && preg_match("/^[a-z0-9_.]+([\\s]{1}[a-z0-9_.]|[a-z0-9_.])+$/i", $_POST['desc'])) ? $db->escape(strip_tags(stripslashes($_POST['desc']))) : '';
			if (empty($_POST['name']) || empty($_POST['desc']) || empty($cost) || empty($days) || empty($cost) || empty($energy) || empty($str) || empty($agil) || empty($gua) || empty($lab) || empty($iq)) {
				echo 'Something went wrong.<br />
            &gt; <a href="staff.php">Goto Main</a>';
				die($h->endpage());
			}
			staff_csrf_stdverify('staff_editcourse2', 'staff_courses.php?action=editcourse');
			$db->query("UPDATE `courses`
                 ...
                 `crIQ` = $iq
                 WHERE `crID` = {$_POST['id']}");
			echo 'Course ' . $_POST['name'] . ' was edited successfully.<br />
                &gt; <a href="staff.php">Goto Main</a>';
			stafflog_add("Edited course {$_POST['name']}");
			die($h->endpage());
			break;
		case "1":
			$_POST['course'] = (isset($_POST['course']) && is_numeric($_POST['course'])) ? abs(intval($_POST['course'])) : '';
			$q               = $db->query("SELECT `crIQ`, `crLABOUR`, `crGUARD`, `crAGIL`,
			...
			staff_csrf_stdverify('staff_editcourse1', 'staff_courses.php?action=editcourse');
			$old = $db->fetch_row($q);
			$db->free_result($q);
...
function delcourse()
{
	...
	$_POST['course'] = (isset($_POST['course']) && is_numeric($_POST['course'])) ? abs(intval($_POST['course'])) : '';
	if ($_POST['course']) {
		staff_csrf_stdverify('staff_delcourse', 'staff_courses.php?action=delcourse');
		$q = $db->query("SELECT `crNAME`
                         FROM `courses`
                         WHERE `crID` = {$_POST['course']}");
		if ($db->num_rows($q) == 0) {
			$db->free_result($q);
		...
		$db->query("DELETE FROM `courses`
        		 WHERE `crID` = {$_POST['course']}");
		echo 'Course ' . $old['crNAME'] . ' deleted.<br />
                &gt; <a href="staff.php">Goto Main</a>';
        ...
        <form action='staff_courses.php?action=delcourse' method='post'>
        	Course: " . course_dropdown(NULL, "course") . "<br />
            {$csrf}
