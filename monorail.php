...
if (empty($_GET['to'])) {
	echo '
	Welcome to the Monorail Station. It costs ' . money_formatter($cost_of_travel) . ' for a ticket.
	<br />
	Where would you like to travel today?
	<br />
   	';
	$q = $db->query("SELECT `cityid`, `cityname`, `citydesc`, `cityminlevel`
                     FROM `cities`
	...
	if ($ir['money'] < $cost_of_travel) {
		...
		if (!$db->num_rows($q)) {
			...
			echo 'Congratulations, you paid ' . money_formatter($cost_of_travel) . ' and travelled to ' . $cityName . ' on the monorail!';
		}
		$db->free_result($q);
	}
