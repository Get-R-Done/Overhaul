...
if (!$fp) {
	...
	while (!feof($fp)) {
		...
		if (strcmp($res, "VERIFIED") == 0) {
			...
			$pack = $packr[2];
			if ($pack != 1 and $pack != 2 and $pack != 3 and $pack != 4 and $pack != 5) {
				fclose($fp);
				die("");
			}
			if (($pack == 1 || $pack == 2 || $pack == 3) && $payment_amount != "3.00") {
				fclose($fp);
				die("");
			}
			...
			$db->query("INSERT INTO `dps_accepted`
                     VALUES(NULL, {$buyer}, {$for}, '$t', " . time() . ", '$txn_db')");
