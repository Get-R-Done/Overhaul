...
if (!$_GET['ID']) {
	...
	$gq       = $db->query("SELECT `gangPRESIDENT`, `gangVICEPRES`, `gangNAME`,
	...
	switch ($_GET['action']) {
		...
		default:gang_view();break;
	}
}
function gang_view()
{
	global $db, $ir, $c, $h, $gangdata;
	$pq = $db->query("SELECT `userid`, `username`
    				 FROM `users`
	...
	$db->free_result($pq);
	$vpq = $db->query("SELECT `userid`, `username`
                     FROM `users`
	...
	if ($coldr['userid'] > 0) {
		print "Vice-President: <a href='viewuser.php?u={$coldr['userid']}'>{$coldr['username']}</a><hr />";
	} else {
	...
	echo "<b>Members:</b> " . $db->fetch_single($cnt) . "<br />
...
function gang_userlist()
{
	...
	$q = $db->query("SELECT `userid`, `username`, `level`, `daysingang`
                     FROM `users`
                     WHERE `gang` = {$gangdata['gangID']}
...
function gang_applysubmit()
{
	...
	if (!$ir['gang']) {
		...
		$gev = $db->escape("<a href='viewuser.php?u={$userid}'>{$ir['username']}</a>" . " sent an application to join this gang.");
