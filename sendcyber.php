...
if (!((int) $_GET['ID'])) {
	...
	$it = $db->query("SELECT `cybermoney`, `lastip`, `username`
                     FROM `users`
	...
	if ($er['cybermoney'] == -1 || $ir['cybermoney'] == -1) {
		die("Sorry,you or the person you are sending to does not have a cyber bank account.");
	}
	if ((int) $_POST['xfer']) {
		if (!isset($_POST['verf']) || !verify_csrf_code("sendcyber_{$_GET['ID']}", stripslashes($_POST['verf']))) {
			echo '<h3>Error</h3><hr />
			...
			event_add($_GET['ID'], "You received " . money_formatter($_POST['xfer']) . " into your cyber bank account from {$ir['username']}.", $c);
			$db->query("INSERT INTO `bankxferlogs`
                     VALUES (NULL, $userid, {$_GET['ID']},
                     {$_POST['xfer']}, " . time() . ", '{$ir['lastip']}',
                     '{$er['lastip']}', 'cyber')");
		}
	} else {
		...
		<br />You have <b>" . money_formatter($ir['cybermoney']) . "</b> you can send.
