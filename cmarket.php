...
function cmarket_index()
{
	...
	$sql = "SELECT `cm`.`cmADDER`, `cm`.`cmPRICE`, `cmID`, `cmQTY`,
	...
	while ($r = $db->fetch_row($q)) {
		...
		<td>{$r['cmQTY']}</td>
		<td> " . money_formatter($r['cmPRICE']) . "</td> <td>" . money_formatter($each) . "</td> <td>[{$link}]
		</td> </tr>";
	}
	$db->free_result($q);
	echo "
...
function crystal_remove()
{
	...
	$q = $db->query('SELECT `cmQTY` FROM `crystalmarket` WHERE `cmID` = ' . $_GET['ID'] . ' AND `cmADDER` = ' . $userid);
	if (!$db->num_rows($q)) {
...
function crystal_buy()
{
	...
	$q = $db->query('SELECT `cmPRICE`, `cmQTY`, `cmADDER` FROM `crystalmarket` WHERE `cmID` = ' . $_GET['ID']);
	if (!$db->num_rows($q)) {
		echo '
	...
	';
		...
		$db->query('UPDATE `users` SET `crystals` = `crystals` + ' . $_POST['QTY'] . ', `money` = `money` - ' . $cprice . ' WHERE `userid` = ' . $userid);
		if ($_POST['QTY'] < $r['cmQTY']) {
			$db->query('UPDATE `crystalmarket` SET `cmQTY` = `cmQTY` - ' . $_POST['QTY'] . ' WHERE `cmID` = ' . $_GET['ID']);
		} elseif ($_POST['QTY'] == $r['cmQTY']) {
			$db->query('DELETE FROM `crystalmarket` WHERE `cmID` = ' . $_GET['ID']);
		}
		$db->query('UPDATE `users` SET `money` = `money` + ' . $cprice . ' WHERE `userid` = ' . $r['cmADDER']);
		event_add($r['cmADDER'], "<a href='viewuser.php?u=$userid'>{$ir['username']}</a> bought of {$_POST['QTY']} your crystals from the market for " . money_formatter($cprice) . ".", $c);
		echo '
	You bought the ' . $_POST['QTY'] . ' crystals from the market for $' . number_format($cprice) . '.
	<br />
	><a href="cmarket.php">Back</a>
	';
...
<br />
     ";
		echo '
 <form action="cmarket.php?action=buy&ID=' . $_GET['ID'] . '" method="post">
...
{
	global $db, $ir, $c, $userid, $h;
	$_POST['amnt']  = (isset($_POST['amnt']) && is_numeric($_POST['amnt'])) ? abs(intval($_POST['amnt'])) : '';
	...
	if (!empty($_POST['amnt']) && !empty($_POST['price'])) {
		...
		$ql = $db->query('SELECT `cmID` FROM `crystalmarket` WHERE cmADDER = ' . $userid . ' AND cmPRICE = ' . $_POST['price']);
		if ($db->num_rows($ql)) {
			$gc = $db->fetch_row($ql);
			$db->free_result($ql);
			$db->query('UPDATE `crystalmarket` SET `cmQTY` = `cmQTY` + ' . $_POST['amnt'] . ' WHERE `cmID` = ' . $gc['cmID']);
		} else {
			$db->free_result($ql);
			$tp = $_POST['price'];
