...
{
	global $db, $ir, $c, $userid, $h;
	if (!isset($_GET['verf']) || !verify_csrf_code('prefs_sexchange', stripslashes($_GET['verf']))) {
...
{
	global $db, $ir, $c, $userid, $h;
	if (!isset($_POST['verf']) || !verify_csrf_code('prefs_passchange', stripslashes($_POST['verf']))) {
...
function do_name_change()
{
	...
	$check_ex = $db->query('SELECT `userid`
                     FROM `users`
...
function do_pic_change()
{
	...
	if (!empty($npic)) {
		if (strlen($npic) < 8 || !(substr($npic, 0, 7) == 'http://' || substr($npic, 0, 8 == 'https://'))) {
...
{
	global $db, $ir, $c, $userid, $h;
	if (!isset($_POST['verf']) || !verify_csrf_code('prefs_forumchange', stripslashes($_POST['verf']))) {
		csrf_error('forumchange');
	}
	$av = (isset($_POST['forums_avatar']) && is_string($_POST['forums_avatar'])) ? stripslashes($_POST['forums_avatar']) : '';
	if (!empty($av)) {
		if (strlen($av) < 8 || !(substr($av, 0, 7) == 'http://' || substr($av, 0, 8 == 'https://'))) {
			echo 'Invalid Image.<br />
	...
	$_POST['forums_signature'] = $db->escape(strip_tags(stripslashes($_POST['forums_signature'])));
