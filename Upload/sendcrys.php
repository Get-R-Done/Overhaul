...
if (!((int) $_GET['ID'])) {
	...
	$it = $db->query("SELECT `lastip`, `username`
    				 FROM `users`
	...
	if ((int) $_POST['crystals']) {
		if (!isset($_POST['verf']) || !verify_csrf_code("sendcrys_{$_GET['ID']}", stripslashes($_POST['verf']))) {
			echo '<h3>Error</h3><hr />
			...
			$db->query("INSERT INTO `crystalxferlogs`
                     VALUES (NULL, $userid, {$_GET['ID']},
                     {$_POST['crystals']}, " . time() . ", '{$ir['lastip']}', '{$er['lastip']}')");
		}
	} else {
		...
		while ($r = $db->fetch_row($q)) {
			echo "<tr>
                    ...
                    <td> " . number_format($r['cxAMOUNT']) . " crystals</td>
                  </tr>";
		}
		$db->free_result($q);
		echo "</table>";
