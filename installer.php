...
function install()
{
	...
	$adm_gender   = (isset($_POST['gender']) && in_array($_POST['gender'], array(
		'Male',
		'Female'
	...
	$db_driver    = (isset($_POST['driver']) && in_array($_POST['driver'], array(
		'mysql',
		'mysqli'
...
EOF;
	...
	$db->query("INSERT INTO `users`
             ...
             '{$adm_gender}', " . time() . ", '{$ins_email}', -1, '$IP', '$IP',
             '{$e_salt}')");
	$i = $db->insert_id();
    ...
    * * * * * php $path/cron_minute.php $code
	...
	if (!$success) {
		...
		if ($success2) {
			echo "<span style='font-weight: bold;'>" . "You should now remove installer.php from your server." . "</span>";
		} else {
			echo "<span style='font-weight: bold; font-size: 20pt;'>" . "YOU MUST REMOVE installer.php " . "from your server.<br />" . "Failing to do so will allow other people " . "to run the installer again and potentially " . "mess up your game entirely." . "</span>";
		}
	} else {
		require_once('installer_foot.php');
