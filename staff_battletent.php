...
function addbot()
{
	...
	if ($_POST['userid'] && $_POST['money']) {
		staff_csrf_stdverify('staff_addbot', 'staff_battletent.php?action=addbot');
		$q = $db->query("SELECT `user_level`, `userid`, `username`
                         FROM `users`
		...
		$q2 = $db->query("SELECT COUNT(`cb_npcid`)
                         FROM `challengebots`
		...
		$db->free_result($q2);
		$db->query("INSERT INTO `challengebots`
                 VALUES('{$r['userid']}', '{$_POST['money']}')");
		echo 'Challenge Bot ' . $r['username'] . ' added.<br />
                &gt; <a href="staff.php">Goto Main</a>';
...
{
	global $db, $ir, $c, $h, $userid;
	$_GET['step'] = (isset($_GET['step']) && in_array($_GET['step'], array(1,2,3))) ? abs(intval($_GET['step'])) : '';
	switch ($_GET['step']) {
		case "2":
			$_POST['userid'] = (isset($_POST['userid']) && is_numeric($_POST['userid'])) ? abs(intval($_POST['userid'])) : '';
			$_POST['money']  = (isset($_POST['money']) && is_numeric($_POST['money'])) ? abs(intval($_POST['money'])) : '';
			...
			staff_csrf_stdverify('staff_editbot_2', 'staff_battletent.php?action=editbot');
			...
			$db->free_result($q2);
			$db->query("UPDATE `challengebots`
                 SET `cb_money` = {$_POST['money']}
                 WHERE `cb_npcid` = {$r['userid']}");
			echo 'Challenge Bot ' . $r['username'] . ' was updated.<br />&gt; <a href="staff.php">Goto Main</a>';
		...
		case "1":
			...
			staff_csrf_stdverify('staff_editbot_1', 'staff_battletent.php?action=editbot');
...
function delbot()
{
	global $db, $ir, $c, $h, $userid;
	$_POST['userid'] = (isset($_POST['userid']) && is_numeric($_POST['userid'])) ? abs(intval($_POST['userid'])) : '';
	$_POST['delcb']  = (isset($_POST['delcb']) && $_POST['delcb'] == 'Yes') ? $_POST['delcb'] : '';
	if (!empty($_POST['userid'])) {
		staff_csrf_stdverify('staff_delbot', 'staff_battletent.php?action=delbot');
		$q = $db->query("SELECT `username`, `userid`
                         FROM `users`
                         WHERE `userid` = {$_POST['userid']}");
		if ($db->num_rows($q) == 0) {
		...
		$db->free_result($q2);
		$db->query("DELETE FROM `challengebots`
                 WHERE `cb_npcid` = {$r['userid']}");
		if ($_POST['delcb'] == 'Yes') {
			$db->query("DELETE FROM `challengesbeaten`
                     WHERE `npcid` = {$r['userid']}");
		}
		echo 'Challenge Bot ' . $r['username'] . ' removed.<br />
              &gt; <a href="staff.php">Goto Main</a>';
