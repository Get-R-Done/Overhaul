...
function imarket_index()
{
   ...
   ';
	$q     = $db->query("SELECT `imPRICE`, `imQTY`, `imCURRENCY`, `imADDER`,
...
function itemm_remove()
{
	...
	$db->query("DELETE FROM `itemmarket`
    			WHERE `imID` = {$_GET['ID']}");
	$imr_log = $db->escape("{$ir['username']} removed {$r['itmname']} x {$r['imQTY']}" . ' from the item market.');
	$db->query("INSERT INTO `imremovelogs`
...
function item_buy()
{
	...
	if ($_GET['ID'] && !$_POST['QTY']) {
		...
		<br />
		<form action="itemmarket.php?action=buy&ID=' . $_GET['ID'] . '" method="post">
		...
		if ($_POST['QTY'] > $r['imQTY']) {
			...
			<br />
			&gt; <a href="itemmarket.php?action=buy&ID=' . $_GET['ID'] . '">Back</a>
			';
			die($h->endpage());
		}
		...
		if ($curr == "money") {
			event_add($r['imADDER'], "<a href='viewuser.php?u=$userid'>{$ir['username']}</a>" . " bought your {$r['itmname']} item " . ' from the market for ' . money_formatter($final_price) . ".", $c);
			$imb_log = $db->escape("{$ir['username']} bought {$r['itmname']} x{$r['imQTY']}" . " from the item market for " . money_formatter($final_price) . " from user ID {$r['imADDER']}");
			$db->query("INSERT INTO `imbuylogs`
                     VALUES (NULL, {$r['imITEM']}, {$r['imADDER']}, $userid,
                     {$final_price}, {$r['imID']}, {$i}, " . time() . ", '{$imb_log}')");
			echo "
			You bought the {$r['itmname']} x{$_POST['QTY']} from the market for " . money_formatter($final_price) . ".";
		} else {
			event_add($r['imADDER'], "<a href='viewuser.php?u=$userid'>{$ir['username']}</a>" . " bought your {$r['itmname']} item " . ' from the market for ' . number_format($final_price) . " crystals.", $c);
			$imb_log = $db->escape("{$ir['username']} bought {$r['itmname']} x{$r['imQTY']}" . " from the item market for " . number_format($final_price) . " crystals from user ID {$r['imADDER']}");
			$db->query("INSERT INTO `imbuylogs`
                     VALUES (NULL, {$r['imITEM']}, {$r['imADDER']}, $userid,
                     {$final_price}, {$r['imID']}, {$i}, " . time() . ", '{$imb_log}')");
			echo "
			You bought the {$r['itmname']} x{$_POST['QTY']} from the market for " . number_format($final_price) . " crystals.";
		}
	}
}
function item_gift1()
{
	...
	$q = $db->query("SELECT `imCURRENCY`, `imPRICE`, `imQTY`, `itmname`
                     FROM `itemmarket` AS `im`
                     INNER JOIN `items` AS `i`
                     ON `i`.`itmid` = `im`.`imITEM`
                     WHERE `im`.`imID` = {$_GET['ID']}");
	...
	if ($curr == "money") {
		echo "
		Buying the <b>{$r['itmname']}</b> for " . money_formatter($r['imPRICE']) . " each as a gift.";
	} else {
		echo "
		Buying the <b>{$r['itmname']}</b> for " . number_format($r['imPRICE']) . " crystals each as a gift.";
	}
...
function item_gift2()
{
	...
	if (!isset($_POST['verf']) || !verify_csrf_code("imgift_{$_POST['ID']}", stripslashes($_POST['verf']))) {
		csrf_error('gift1&amp;ID=' . $_GET['ID']);
	}
	$query_user_exist = $db->query("SELECT COUNT(`userid`)
                     FROM `users`
	...
	if ($_POST['QTY'] > $r['imQTY']) {
		...
		Error, you cannot buy more than <b>' . $r['imQTY'] . ' ' . $r['itmname'] . '(s)</b>
		<br />
		&gt; <a href="itemmarket.php?action=buy&ID=' . $_POST['ID'] . '">Back</a>
		';
		die($h->endpage());
	}
	...
	$db->query("UPDATE `users`
             ...
             WHERE `userid` = {$r['imADDER']}");
	if ($curr == "money") {
		event_add($r['imADDER'], "<a href='viewuser.php?u=$userid'>{$ir['username']}</a>" . " bought your {$r['itmname']} x{$_POST['QTY']} item(s)" . " from the market for " . money_formatter($final_price) . ".", $c);
		event_add($_POST['user'], "<a href='viewuser.php?u=$userid'>{$ir['username']}</a>" . " bought you {$r['itmname']} x{$_POST['QTY']}" . " from the item market as a gift.", $c);
		$u     = $db->query("SELECT `username`
                         FROM `users`
                         WHERE `userid` = {$_POST['user']}");
		$uname = $db->fetch_single($u);
		$db->free_result($u);
		$img_log = $db->escape("{$ir['username']} bought {$r['itmname']} x{$r['imQTY']} from the item market for " . money_formatter($final_price) . " from user ID {$r['imADDER']} as a gift for $uname [{$_POST['user']}]");
		$db->query("INSERT INTO `imbuylogs`
                 VALUES (NULL, {$r['imITEM']}, {$r['imADDER']}, $userid,
                 {$final_price}, {$r['imID']}, $i, " . time() . ", '{$img_log}')");
		echo "
		You bought the {$r['itmname']} from the market for " . money_formatter($final_price) . " and sent the gift to $uname.
		";
	} else {
		event_add($r['imADDER'], "<a href='viewuser.php?u=$userid'>{$ir['username']}</a>" . " bought your {$r['itmname']} x{$_POST['QTY']} item(s)" . " from the market for " . number_format($final_price) . " crystals.", $c);
		event_add($_POST['user'], "<a href='viewuser.php?u=$userid'>{$ir['username']}</a>" . " bought you {$r['itmname']} x{$_POST['QTY']}" . " from the item market as a gift.", $c);
		$u     = $db->query("SELECT `username`
                         FROM `users`
                         WHERE `userid` = {$_POST['user']}");
		$uname = $db->fetch_single($u);
		$db->free_result($u);
		$img_log = $db->escape("{$ir['username']} bought {$r['itmname']} x{$r['imQTY']} from the item market for " . number_format($final_price) . " crystals from user ID {$r['imADDER']} as a gift for $uname [{$_POST['user']}]");
		$db->query("INSERT INTO `imbuylogs`
                 VALUES (NULL, {$r['imITEM']}, {$r['imADDER']}, $userid,
                 {$final_price}, {$r['imID']}, $i, " . time() . ", '{$img_log}')");
		echo "
		You bought the {$r['itmname']} from the market for " . number_format($final_price) . " crystals and sent the gift to $uname.
		";
	}
}
$h->endpage();
