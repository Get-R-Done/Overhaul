...
if (!$ir['job']) {
	if (!$_GET['interview']) {
		...
		<br />
   		";
		$q = $db->query("SELECT `jID`,`jDESC`,`jNAME`
        				 FROM `jobs`");
		...
		$q = $db->query("SELECT `jOWNER`, `jrID`, `jrIQN`, `jrLABOURN`,
                         `jrSTRN`
                         FROM `jobs` AS `j`
...
function job_index()
{
    ...
    <table width='75%' cellspacing='1' class='table'>
   	...
   	";
	$q = $db->query("SELECT `jrNAME`, `jrPAY`, `jrSTRN`, `jrIQN`, `jrLABOURN`
                     FROM `jobranks`
                     WHERE `jrJOB` = {$ir['job']}
                     ORDER BY `jrPAY` ASC");
	while ($r = $db->fetch_row($q)) {
