...
if ($ir['cybermoney'] > -1) {
	...
	if (isset($_GET['buy'])) {
		if ($ir['money'] >= $bank_cost) {
			echo "Congratulations, you bought a bank account for " . money_formatter($bank_cost) . "!<br />
...
<a href='explore.php'>Back to town...</a>";
		...
		echo "Open a bank account today, just " . money_formatter($bank_cost) . "!<br />
...
The maximum fee is " . money_formatter($bank_maxfee_dp) . ".<form action='cyberbank.php?action=deposit' method='post'>
...
The maximum fee is " . money_formatter($bank_maxfee_wd) . ".<form action='cyberbank.php?action=withdraw' method='post'>
...
function deposit()
{
	...
	$_POST['deposit'] = (isset($_POST['deposit']) && is_numeric($_POST['deposit'])) ? abs((int) $_POST['deposit']) : 0;
	if ($_POST['deposit'] > $ir['money']) {
		...
		$db->query("UPDATE `users`
                ...
                WHERE `userid` = $userid");
		echo "You hand over " . money_formatter($_POST['deposit']) . " to be deposited, <br />
after the fee is taken (" . money_formatter($fee) . "), " . money_formatter($gain) . " is added to your account. <br />
<b>You now have " . money_formatter($ir['cybermoney']) . " in the Cyber Bank.</b><br />
...
function withdraw()
{
	...
	$_POST['withdraw'] = (isset($_POST['withdraw']) && is_numeric($_POST['withdraw'])) ? abs((int) $_POST['withdraw']) : 0;
	if ($_POST['withdraw'] > $ir['cybermoney']) {
		...
		$db->query("UPDATE `users`
                ...
                `money` = `money` + $gain
        		WHERE `userid` = $userid");
		echo "You ask to withdraw " . money_formatter($gain) . ", <br />
