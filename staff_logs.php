...
function view_attack_logs()
{
	...
	for ($i = 1; $i <= $pages; $i++) {
		$s = ($i - 1) * $app;
		echo ($s == $st) ? '<b>' . $i . '</b>&nbsp;' : '<a href="staff_logs.php?action=atklogs&st=' . $s . '">' . $i . '</a>&nbsp;';
		echo ($i % 25 == 0) ? '<br />' : '';
	}
	echo "
    <br />
    <table width='100%' cellspacing='1' cellpadding='1' class='table'>
       ...
       ";
	$q = $db->query("SELECT `stole`, `result`, `attacked`, `attacker`, `time`,
	...
	while ($r = $db->fetch_row($q)) {
		...
		if ($r['result'] == "won") {
			...
			if ($r['stole'] == -1) {
				...
				echo "{$r['un_attacker']} mugged " . money_formatter($r['stole']) . " from {$r['un_attacked']}";
			}
			echo '</td>';
		} else {
			echo "
...
function view_itm_logs()
{
    ...
    <table width='100%' cellspacing='1' cellpadding='1' class='table'>
       ...
       ";
	$q = $db->query("SELECT `ixTO`, `ixFROM`, `ixQTY`, `ixTIME`, `ixTOIP`,
                     ...
                     LIMIT $st, $app");
	while ($r = $db->fetch_row($q)) {
		$same = ($r['ixFROMIP'] == $r['ixTOIP']) ? '<span style="color: red;">Yes</span>' : '<span style="color: green;">No</span>';
		echo "
		<tr>
        	<td>" . date('F j Y, g:i:s a', $r['ixTIME']) . "</td>
	...
	for ($i = 1; $i <= $pages; $i++) {
		$s = ($i - 1) * $app;
		echo ($s == $st) ? '<b>' . $i . '</b>&nbsp;' : '<a href="staff_logs.php?action=itmlogs&st=' . $s . '">' . $i . '</a>&nbsp;';
		echo ($i % 25 == 0) ? '<br />' : '';
	}
	$mypage = floor($_GET['st'] / 100) + 1;
...
function view_cash_logs()
{
	...
	for ($i = 1; $i <= $pages; $i++) {
		$s = ($i - 1) * $app;
		echo ($s == $st) ? '<b>' . $i . '</b>&nbsp;' : '<a href="staff_logs.php?action=cashlogs&st=' . $s . '">' . $i . '</a>&nbsp;';
		echo ($i % 25 == 0) ? '<br />' : '';
	}
	echo "
    <br />
    <table width='100%' cellspacing='1' cellpadding='1' class='table'>
       ...
       ";
	$q = $db->query("SELECT `cxAMOUNT`, `cxTO`, `cxFROM`, `cxTIME`, `cxID`,
                     ...
                     LIMIT $st, $app");
	while ($r = $db->fetch_row($q)) {
		$m = ($r['cxFROMIP'] == $r['cxTOIP']) ? '<span style="color: red; font-weight: bold;">MULTI</span>' : '';
		echo "
		<tr>
        	<td>{$r['cxID']}</td>
        	<td>" . date("F j, Y, g:i:s a", $r['cxTIME']) . "</td>
        	<td>
        	...
        	<td>$m</td>
        	<td> " . money_formatter($r['cxAMOUNT']) . "</td>
        	<td>
...
function view_bank_logs()
{
	...
	for ($i = 1; $i <= $pages; $i++) {
		$s = ($i - 1) * $app;
		echo ($s == $st) ? '<b>' . $i . '</b>&nbsp;' : '<a href="staff_logs.php?action=banklogs&st=' . $s . '">' . $i . '</a>&nbsp;';
		echo ($i % 25 == 0) ? '<br />' : '';
	}
	echo "
    <table width='100%' cellspacing='1' cellpadding='1' class='table'>
...
function view_crys_logs()
{
	...
	for ($i = 1; $i <= $pages; $i++) {
		$s = ($i - 1) * $app;
		echo ($s == $st) ? '<b>' . $i . '</b>&nbsp;' : '<a href="staff_logs.php?action=cryslogs&st=' . $s . '">' . $i . '</a>&nbsp;';
		echo ($i % 25 == 0) ? '<br />' : '';
	}
	echo "
    <table width='100%' cellspacing='1' cellpadding='1' class='table'>
...
function view_mail_logs()
{
	...
	for ($i = 1; $i <= $pages; $i++) {
		$s = ($i - 1) * $app;
		echo ($s == $st) ? '<b>' . $i . '</b>&nbsp;' : '<a href="staff_logs.php?action=maillogs&st=' . $s . '">' . $i . '</a>&nbsp;';
		echo ($i % 25 == 0) ? '<br />' : '';
	}
	echo "
    <table width='100%' cellspacing='1' cellpadding='1' class='table'>
    ...
    ";
	$q = $db->query("SELECT `mail_text`, `mail_subject`, `mail_to`,
...
function view_staff_logs()
{
	...
	for ($i = 1; $i <= $pages; $i++) {
		$s = ($i - 1) * $app;
		echo ($s == $st) ? '<b>' . $i . '</b>&nbsp;' : '<a href="staff_logs.php?action=stafflogs&st=' . $s . '">' . $i . '</a>&nbsp;';
		echo ($i % 25 == 0) ? '<br />' : '';
	}
	echo "
    <table width='100%' cellspacing='1' cellpadding='1' class='table'>
       ...
       ";
	$q = $db->query("SELECT `ip`, `time`, `action`, `user`, `u`.`username`
                     FROM `stafflog` AS `s`
