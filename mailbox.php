...
OUT;
	$q = $db->query("SELECT `m`.*, `userid`, `username`
                     FROM `mail` AS `m`
...
function mail_compose()
{
	...
	$user_exists = false;
	if ($_GET['ID']) {
		$un_query = $db->query("SELECT `username`
        				 FROM `users`
	...
	if ($user_exists) {
		...
		$q = $db->query("SELECT `mail_time`, `mail_text`, `mail_from`
                         FROM `mail`
...
{
	global $db, $ir, $c, $userid, $h;
	if (!isset($_GET['verf']) || !verify_csrf_code('mailbox_delall', stripslashes($_GET['verf']))) {
		echo '<h3>Error</h3><hr />
	...
	$m_c = $db->query("SELECT COUNT(`mail_id`)
                     FROM `mail`
                     WHERE `mail_to` = {$userid}");
