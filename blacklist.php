...
<br />
Most hated: [";
	$q2r = $db->query("SELECT `username`, `userid` FROM `users` ORDER BY `enemy_count` DESC LIMIT 5");
	$r   = 0;
...
<table width="90%" class="table" cellspacing="1">
   ...
   ';
	$q = $db->query("SELECT `bl`.`bl_COMMENT`, `bl_ID`,
	...
	while ($r = $db->fetch_row($q)) {
		$on = ($r['laston'] >= (($_SERVER['REQUEST_TIME'] - 15) * 60)) ? '<font color="green"><b>Online</b></font>' : '<font color="red"><b>Offline</b></font>';
		$d  = "";
		if ($r['donatordays']) {
	...
	<td>" . strip_tags($r['bl_COMMENT']) . "</td> <td><a href='blacklist.php?action=ccomment&b={$r['bl_ID']}'>Change</a></td>
	<td>$on</td>
		</tr>
   ";
	}
...
function add_enemy()
{
	...
	if ($_POST['ID']) {
		$qc         = $db->query("SELECT COUNT(`bl_ADDER`) FROM `blacklist` WHERE `bl_ADDER` = $userid AND `bl_ADDED` = {$_POST['ID']}");
		$q          = $db->query("SELECT `username` FROM `users` WHERE `userid` = {$_POST['ID']}");
...
function remove_enemy()
{
	...
	$_GET['b'] = (isset($_GET['b']) && is_numeric($_GET['b'])) ? abs(intval($_GET['b'])) : '';
	if (empty($_GET['b'])) {
		echo '
...
&gt; <a href="blacklist.php">Back</a>
   ';
	...
	$q = $db->query("SELECT `bl_ADDED` FROM `blacklist` WHERE `bl_ID` = {$_GET['b']} AND `bl_ADDER` = $userid");
	if ($db->num_rows($q) == 0) {
	...
	$r = $db->fetch_row($q);
	$db->free_result($q);
	$db->query("DELETE FROM `blacklist` WHERE `bl_ID` = {$_GET['b']} AND `bl_ADDER` = $userid");
...
{
	global $db, $ir, $c, $userid, $h;
	$_POST['b']       = (isset($_POST['b']) && is_numeric($_POST['b'])) ? abs(intval($_POST['b'])) : '';
	$_POST['comment'] = (isset($_POST['comment']) && is_string($_POST['comment'])) ? $db->escape(strip_tags(stripslashes($_POST['comment']))) : '';
	if (!empty($_POST['comment']) && !empty($_POST['b'])) {
		$db->query("UPDATE `blacklist` SET `bl_COMMENT` = '{$_POST['comment']}' WHERE `bl_ID` = {$_POST['b']} AND `bl_ADDER` = $userid");
		echo "
...
<a href='blacklist.php'>&gt; Back</a>
   ";
		...
		$q = $db->query("SELECT `bl_COMMENT` FROM `blacklist` WHERE `bl_ID` = {$_GET['b']} AND `bl_ADDER` = $userid");
		if ($db->num_rows($q) > 0) {
			$r       = $db->fetch_row($q);
