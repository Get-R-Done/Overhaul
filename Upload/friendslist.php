...
switch ($_GET['action']) {
	case "add":add_friend();break;
	case "remove":remove_friend();break;
	case "ccomment":change_comment();break;
	default:friends_list();break;
}
...
<br />
Most hated: [";
	$q2r = $db->query("SELECT `username`, `userid`
                     FROM `users`
...
<table width="90%" class="table" cellspacing="1">
   ...
   ';
	$q = $db->query("SELECT `fl_COMMENT`, `fl_ID`, `laston`, `donatordays`,
                     `username`, `userid`
	...
	while ($r = $db->fetch_row($q)) {
		$on = ($r['laston'] >= (($_SERVER['REQUEST_TIME'] - 15) * 60)) ? '<span style="color: green; font-weight: bold;">Online</font>' : '<span style="color: red; font-weight: bold;">Offline</font>';
		$d  = "";
		if ($r['donatordays'] > 0) {
...
function add_friend()
{
	...
	if ($_POST['ID']) {
		$qc         = $db->query("SELECT COUNT(`fl_ADDER`)
                         FROM `friendslist`
		...
		if ($dupe_count > 0) {
			...
			$db->query("INSERT INTO `friendslist`
...
&gt; <a href="friendslist.php">Back</a>
   ';
	...
	$r = $db->fetch_row($q);
	$db->query("DELETE FROM `friendslist`
            WHERE `fl_ID` = {$_GET['f']} AND `fl_ADDER` = $userid");
	$db->query("UPDATE `users`
             SET `friend_count` = `friend_count` - 1
...
{
	global $db, $ir, $c, $userid, $h;
	$_POST['f']       = (isset($_POST['f']) && is_numeric($_POST['f'])) ? abs(intval($_POST['f'])) : '';
	$_POST['comment'] = $db->escape(strip_tags(stripslashes($_POST['comment'])));
	if ($_POST['comment'] && $_POST['f']) {
		$q = $db->query("SELECT COUNT(`fl_ID`)
                     FROM `friendslist`
                     WHERE `fl_ID` = {$_GET['f']} AND `fl_ADDER` = $userid");
...
<a href='friendslist.php'>&gt; Back</a>
   ";
		...
		$q = $db->query("SELECT `fl_COMMENT`
                         FROM `friendslist`
                         WHERE `fl_ID` = {$_GET['f']}
                         AND `fl_ADDER` = $userid");
		if ($db->num_rows($q)) {
			$r       = $db->fetch_row($q);
			$comment = stripslashes(htmlentities($r['fl_COMMENT'], ENT_QUOTES, 'ISO-8859-1'));
			echo "
Changing a comment.
