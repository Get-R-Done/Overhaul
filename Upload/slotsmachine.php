...
if (($_SESSION['tresde'] == $_GET['tresde']) || $_GET['tresde'] < 100) {
	die("Error, you cannot refresh or go back on the slots,
...
if (isset($_POST['bet']) && is_numeric($_POST['bet'])) {
	...
	$slot[2] = (int) rand(0, 9);
	$slot[3] = (int) rand(0, 9);
	echo "You place " . money_formatter($_POST['bet']) . " into the slot and pull the pole.<br />
	You see the numbers: <b>$slot[1] $slot[2] $slot[3]</b><br />
	You bet " . money_formatter($_GET['bet']) . " ";
	if ($slot[1] == $slot[2] && $slot[2] == $slot[3]) {
		$won  = $_POST['bet'] * 26;
		$gain = $_POST['bet'] * 25;
		echo "and won " . money_formatter($won) . " by lining up 3 numbers pocketing you " . money_formatter($gain) . " extra.";
	} else if ($slot[1] == $slot[2] || $slot[2] == $slot[3] || $slot[1] == $slot[3]) {
		$won  = $_POST['bet'] * 3;
		$gain = $_POST['bet'] * 2;
		echo "and won " . money_formatter($won) . " by lining up 2 numbers pocketing you " . money_formatter($gain) . " extra.";
	} else {
		$won  = 0;
		$gain = -$_POST['bet'];
		echo "and lost it.";
	...
	The maximum bet for your level is " . money_formatter($maxbet) . ".<br />
	<form action='slotsmachine.php?tresde={$tresder}' method='POST'>
