...
function new_item_submit()
{
	...
	$itmname               = (isset($_POST['itmname']) && preg_match("/^[a-z0-9_]+([\\s]{1}[a-z0-9_]|[a-z0-9_])+$/i", $_POST['itmname'])) ? $db->escape(strip_tags(stripslashes($_POST['itmname']))) : '';
	$itmdesc               = (isset($_POST['itmdesc'])) ? $db->escape(strip_tags(stripslashes($_POST['itmdesc']))) : '';
	...
	if (empty($itmname) || empty($itmdesc) || empty($_POST['itmtype']) || empty($_POST['itmbuyprice']) || empty($_POST['itmsellprice'])) {
	...
	for ($i = 1; $i <= 3; $i++) {
		$efxkey                    = "effect{$i}";
		$_POST[$efxkey . 'stat']   = (isset($_POST[$efxkey . 'stat']) && in_array($_POST[$efxkey . 'stat'], array(
			'energy','will','brave','hp',
			'strength','agility','guard','labour',
			'IQ','hospital','jail','money',
			'crystals','cdays','bankmoney','cybermoney',
			'crimexp'
		...
		$_POST[$efxkey . 'on']     = (isset($_POST[$efxkey . 'on']) && in_array($_POST[$efxkey . 'on'], array('1','0'))) ? $_POST[$efxkey . 'on'] : 0;
		$effects[$i]               = $db->escape(serialize(array(
			"stat" => $_POST[$efxkey . 'stat'],
			"dir" => $_POST[$efxkey . 'dir'],
			"inc_type" => $_POST[$efxkey . 'type'],
			"inc_amount" => abs((int) $_POST[$efxkey . 'amount'])
		)));
	}
	$m = $db->query("INSERT INTO `items`
	...
	echo 'The ' . $_POST['itmname'] . ' Item was added to the game.<br />
...
function give_item_submit()
{
	...
	$q  = $db->query('SELECT COUNT(`itmid`)
                     FROM `items`
                     WHERE `itmid` = ' . $_POST['item']);
	$q2 = $db->query('SELECT COUNT(`userid`)
                     FROM `users`
	...
	echo 'You gave ' . $_POST['qty'] . ' of item ID ' . $_POST['item'] . ' to user ID ' . $_POST['user'] . '<br />
            &gt; <a href="staff.php">Go Back</a>';
	die($h->endpage());
}
...
function kill_item_submit()
{
	...
	$db->query("DELETE FROM `items`
     WHERE `itmid` = {$_POST['item']}");
	$db->query("DELETE FROM `shopitems`
	...
	echo 'The ' . $itemi['itmname'] . ' Item was removed from the game.<br />
...
function edit_item_form()
{
	...
	for ($i = 1; $i <= 3; $i++) {
		...
		$str  = ($efx['dir'] == "neg") ? '<option value="pos">Increase</option>
                        	<option value="neg" selected="selected">Decrease</option>' : '<option value="pos" selected="selected">Increase</option>
                        	<option value="neg">Decrease</option>';
		$str2 = ($efx['inc_type'] == "percent") ? '<option value="figure">Value</option>
                        	<option value="percent" selected="selected">Percent</option>' : '<option value="figure" selected="selected">Value</option>
                        	<option value="percent">Percent</option>';
		echo "
        </select>
...
function edit_item_sub()
{
	...
	if (empty($itmname) || empty($itmdesc) || empty($_POST['itmtype']) || empty($_POST['itmbuyprice']) || empty($_POST['itmsellprice']) || empty($_POST['itmid'])) {
		echo 'You missed one or more of the fields. Please go back and try again.<br />
        &gt; <a href="staff_items.php?action=edititem">Go Back</a>';
		die($h->endpage());
	}
...
function newitemtype()
{
	...
	if (!empty($_POST['name'])) {
		staff_csrf_stdverify('staff_newitemtype', 'staff_items.php?action=newitemtype');
		$db->query("INSERT INTO `itemtypes`
         		 VALUES(NULL, '{$_POST['name']}')");
		stafflog_add('Added item type ' . $_POST['name']);
		echo 'Item Type ' . $_POST['name'] . ' added.<br />
                &gt; <a href="staff.php">Go Home</a>';
		die($h->endpage());
	} else {
