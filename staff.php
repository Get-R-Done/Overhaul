...
function basicsettings()
{
	...
	$_POST['game_name']        = (isset($_POST['game_name']) && preg_match("/^[a-z0-9_.]+([\\s]{1}[a-z0-9_.]|[a-z0-9_.])+$/i", $_POST['game_name'])) ? $db->escape(strip_tags(stripslashes($_POST['game_name']))) : '';
	$_POST['game_owner']       = (isset($_POST['game_owner']) && preg_match("/^[a-z0-9_]+([\\s]{1}[a-z0-9_]|[a-z0-9_])+$/i", $_POST['game_owner'])) ? $db->escape(strip_tags(stripslashes($_POST['game_owner']))) : '';
	$_POST['game_description'] = (isset($_POST['game_description'])) ? $db->escape(strip_tags(stripslashes($_POST['game_description']))) : '';
	$_POST['paypal']           = (isset($_POST['paypal']) && filter_input(INPUT_POST, 'paypal', FILTER_VALIDATE_EMAIL)) ? $db->escape(stripslashes($_POST['paypal'])) : '';
	...
	$_POST['validate_on']      = (isset($_POST['validate_on']) && in_array($_POST['validate_on'], array('1','0'), true)) ? $_POST['validate_on'] : FALSE;
	$_POST['validate_period']  = (isset($_POST['validate_period']) && in_array($_POST['validate_period'], array('5','15','60','login'), true)) ? $_POST['validate_period'] : FALSE;
	$_POST['regcap_on']        = (isset($_POST['regcap_on']) && in_array($_POST['regcap_on'], array('1','0'), true)) ? $_POST['regcap_on'] : FALSE;
	$_POST['sendcrys_on']      = (isset($_POST['sendcrys_on']) && in_array($_POST['sendcrys_on'], array('1','0'), true)) ? $_POST['sendcrys_on'] : FALSE;
	$_POST['sendbank_on']      = (isset($_POST['sendbank_on']) && in_array($_POST['sendbank_on'], array('1','0'), true)) ? $_POST['sendbank_on'] : FALSE;
	if (empty($_POST['game_name']) || empty($_POST['game_owner']) || empty($_POST['game_description']) || empty($_POST['paypal']) || empty($_POST['ct_refillprice']) || empty($_POST['ct_iqpercrys']) || empty($_POST['ct_moneypercrys']) || is_bool($_POST['validate_on']) || is_bool($_POST['validate_period']) || is_bool($_POST['regcap_on']) || is_bool($_POST['sendcrys_on']) || is_bool($_POST['sendbank_on'])) {
		...
		foreach ($opt as $k => $v) {
			echo ($k == $set['validate_on']) ? '<option value="' . $k . '" selected="selected">' . $v . '</option>' : '<option value="' . $k . '">' . $v . '</option>';
		}
		echo "
        </select>
        <br />
		...
		foreach ($opt as $k => $v) {
			echo ($k == $set['validate_period']) ? '<option value="' . $k . '" selected="selected">' . $v . '</option>' : '<option value="' . $k . '">' . $v . '</option>';
		}
		echo "
        </select>
        <br />
		...
		$opt = array("1" => "On","0" => "Off");
		foreach ($opt as $k => $v) {
			echo ($k == $set['regcap_on']) ? '<option value="' . $k . '" selected="selected">' . $v . '</option>' : '<option value="' . $k . '">' . $v . '</option>';
		}
		echo "
        </select>
        <br />
		...
		$opt = array("1" => "On","0" => "Off");
		foreach ($opt as $k => $v) {
			echo ($k == $set['sendcrys_on']) ? '<option value="' . $k . '" selected="selected">' . $v . '</option>' : '<option value="' . $k . '">' . $v . '</option>';
		}
		echo "
        </select>
        <br />
		...
		$opt = array("1" => "On","0" => "Off");
		foreach ($opt as $k => $v) {
			echo ($k == $set['sendbank_on']) ? '<option value="' . $k . '" selected="selected">' . $v . '</option>' : '<option value="' . $k . '">' . $v . '</option>';
		}
		echo "
        </select>
        <br />
        	...
        	<br />
        	Will Potion Item: " . item_dropdown(NULL, "willp_item", $set['willp_item']) . "<br />
            {$csrf}
...
function announcements()
{
	...
	if (isset($_POST['text']) && !empty($_POST['text'])) {
		...
		$_POST['text'] = $db->escape(htmlentities(stripslashes($_POST['text']), ENT_QUOTES, 'ISO-8859-1'));
...
function index()
{
	...
	if ($ir['user_level'] == 2) {
        ...
        <table width='100%' cellspacing='1' class='table'>
           ...
           ";
		$q = $db->query("SELECT `user`, `action`, `time`, `ip`, `username`
                         FROM `stafflog` AS `s`
		...
		while ($r = $db->fetch_row($q)) {
        	...
        	<tr>
        		...
        		<td>{$r['action']}</td>
        		<td>" . date('F j Y g:i:s a', $r['time']) . "</td>
        		<td>{$r['ip']}</td>
        	</tr>
           	";
		}
	...
	<form action='staff.php' method='post'>
		<textarea rows='10' cols='60' name='pad'>" . htmlentities($set['staff_pad'], ENT_QUOTES, 'ISO-8859-1') . "</textarea>
		<br />
		{$csrf}
