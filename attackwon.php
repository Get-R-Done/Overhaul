...
if ($db->num_rows($od) > 0) {
	...
	if ($r['hp'] == 1) {
		...
		You knock {$r['username']} on the floor a few times to make sure he is unconscious, " . "then open his wallet, snatch " . money_formatter($stole) . ", and run home happily.";
		$hosptime = rand(20, 40) + floor($ir['level'] / 8);
		$expgain  = 0;
		$db->query("UPDATE `users` SET `exp` = `exp` + $expgain, `money` = `money` + $stole WHERE `userid` = $userid");
		...
		event_add($r['userid'], "<a href='viewuser.php?u=$userid'>{$ir['username']}</a> mugged you and stole " . money_formatter($stole) . ".", $c);
		...
		if ($ir['gang'] > 0 && $r['gang'] > 0) {
			$gq = $db->query("SELECT `gangRESPECT`, `gangID` FROM `gangs` WHERE `gangID` = {$r['gang']}");
			if ($db->num_rows($gq) > 0) {
				$ga   = $db->fetch_row($gq);
				$warq = $db->query("SELECT COUNT(`warDECLARER`) FROM `gangwars`
		...
		if ($r['user_level'] == 0) {
			...
			if ($db->num_rows($q) > 0) {
				$cb = $db->fetch_row($q);
				$qk = $db->query("SELECT COUNT(`npcid`) FROM `challengesbeaten`
