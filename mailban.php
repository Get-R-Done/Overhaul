...
$_POST['reason'] = (isset($_POST['reason']) && ((strlen($_POST['reason']) > 3) && (strlen($_POST['reason']) < 50))) ? strip_tags(stripslashes($_POST['reason'])) : '';
...
if (!empty($_POST['user']) && !empty($_POST['reason']) && !empty($_POST['days'])) {
	if (!isset($_POST['verf']) || !verify_csrf_code('mailban', stripslashes($_POST['verf']))) {
		echo '<h3>Error</h3><hr />
   			This operation has been blocked for your security.<br />
    		Please try again.<br />
    		&gt; <a href="mailban.php?userid=' . $_POST['user'] . '">Try Again</a>';
		die($h->endpage());
	}
