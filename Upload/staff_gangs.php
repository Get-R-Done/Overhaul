...
function admin_gang_record()
{
	...
	$_POST['reason'] = (isset($_POST['reason']) && preg_match("/^[a-z0-9_]+([\\s]{1}[a-z0-9_]|[a-z0-9_])+$/i", $_POST['reason'])) ? $db->escape(strip_tags(stripslashes($_POST['reason']))) : '';
	if ($gang) {
		staff_csrf_stdverify('staff_gangs_record', 'staff_gangs.php?action=grecord');
		$q = $db->query("SELECT `gangNAME`, `gangDESC`, `gangPREF`,
		...
		if ($db->num_rows($q) == 0) {
            ...
            Annnouncement: {$r['gangAMENT']}
   			...
   			";
			stafflog_add($ir['username'] . ' looked at gang id ' . $r['gangID'] . ' (' . $r['gangNAME'] . ')\'s record. with the reason ' . $_POST['reason']);
		}
	} else {
...
function admin_gang_credit()
{
	...
	$reason   = (isset($_POST['reason']) && preg_match("/^[a-z0-9_]+([\\s]{1}[a-z0-9_]|[a-z0-9_])+$/i", $_POST['reason'])) ? $db->escape(strip_tags(stripslashes($_POST['reason']))) : '';
	if (($money != 0 || $crystals != 0) && ($gang && $reason)) {
		$q = $db->query("SELECT `gangNAME`
                         FROM `gangs`
                         WHERE `gangID` = $gang");
		if ($db->num_rows($q) == 0) {
			$db->free_result($q);
		...
		staff_csrf_stdverify('staff_gangs_credit2', 'staff_gangs.php?action=gcredit');
		...
		stafflog_add("{$ir['username']} credited {$gangname} (gang ID {$gang})
                 with {$money} money and/or {$crystals} crystals
                 with the reason {$reason}");
	} else if ($gang && ($money != 0 || $crystals != 0)) {
		staff_csrf_stdverify('staff_gangs_credit1', 'staff_gangs.php?action=gcredit');
		$q = $db->query("SELECT `gangNAME`
                         FROM `gangs`
                         WHERE `gangID` = $gang");
		if ($db->num_rows($q) == 0) {
		...
		echo "
        You are crediting " . $db->fetch_single($q) . " with " . money_formatter($money) . " and/or $crystals crystals.
        <br />
...
function admin_gang_wars()
{
	...
	<table width="75%" border="2">
   	';
	$q = $db->query("SELECT `warID`, `warDECLARED`, `warDECLARER`,
...
function admin_gang_wardelete()
{
	...
	staff_csrf_stdverify("staff_gangs_wardelete{$_GET['war']}", 'staff_gangs.php?action=gwar');
	$q = $db->query("SELECT `warDECLARED`, `warDECLARER`,
                     `g1`.`gangNAME` AS `declarer`,
                     `g1`.`gangRESPECT` AS `drespect`,
                     `g2`.`gangNAME` AS `defender`,
                     `g2`.`gangRESPECT` AS `frespect`
	...
	echo 'War cleared.<br />
    &gt; <a href="staff_gangs.php?action=gwar">Go Back</a>';
	stafflog_add("{$ir['username']} deleted war ID {$_GET['war']}
	...
	if ($gang) {
		...
		foreach ($edits as $k => $v) {
			if ($v[2] >= $ir['user_level']) {
				$a = "green'>Yes";
				$l = "<a href='staff_gangs.php?action=$v[1]&amp;gang=$gang'>Go</a>";
			} else {
				$a = "red'>No";
				$l = "N/A";
			}
...
function admin_gang_edit_name()
{
	...
	$gang              = (isset($_GET['gang']) && is_numeric($_GET['gang'])) ? abs(intval($_GET['gang'])) : 0;
	$_POST['gangNAME'] = (isset($_POST['gangNAME']) && preg_match("/^[a-z0-9_]+([\\s]{1}[a-z0-9_]|[a-z0-9_])+$/i", $_POST['gangNAME'])) ? $db->escape(strip_tags(stripslashes($_POST['gangNAME']))) : '';
	$_POST['gangDESC'] = (isset($_POST['gangDESC'])) ? $db->escape(strip_tags(stripslashes($_POST['gangDESC']))) : '';
	$q                 = $db->query("SELECT `gangNAME`,`gangDESC`
                     FROM `gangs`
                     WHERE `gangID` = $gang");
	if ($db->num_rows($q) == 0) {
		$db->free_result($q);
	...
	if ($gang && $_POST['gangNAME'] && $_POST['gangDESC']) {
		staff_csrf_stdverify('staff_gangs_edit_name', "staff_gangs.php?action=gedit_name&amp;gang={$gang}");
		$db->query("UPDATE `gangs`
...
function admin_gang_edit_prefix()
{
	...
	$gang              = (isset($_GET['gang']) && is_numeric($_GET['gang'])) ? abs(intval($_GET['gang'])) : 0;
	$_POST['gangPREF'] = (isset($_POST['gangPREF']) && strlen($_POST['gangPREF']) <= 5) ? $db->escape(strip_tags(stripslashes($_POST['gangPREF']))) : '';
	$q                 = $db->query("SELECT `gangNAME`, `gangPREF`
                     FROM `gangs`
                     WHERE `gangID` = $gang");
	if ($db->num_rows($q) == 0) {
		$db->free_result($q);
	...
	if ($gang && $_POST['gangPREF']) {
		staff_csrf_stdverify('staff_gangs_edit_prefix', "staff_gangs.php?action=gedit_prefix&amp;gang={$gang}");
		$db->query("UPDATE `gangs`
...
function admin_gang_edit_finances()
{
	...
	$q        = $db->query("SELECT `gangNAME`, `gangMONEY`, `gangCRYSTALS`,
                     `gangRESPECT`
                     FROM `gangs`
                     WHERE `gangID` = $gang");
	if ($db->num_rows($q) == 0) {
	...
	$db->free_result($q);
	if ($gang && $reason) {
		staff_csrf_stdverify('staff_gangs_edit_finances', "staff_gangs.php?action=gedit_finances&amp;gang={$gang}");
		$db->query("UPDATE `gangs`
...
function admin_gang_edit_staff()
{
	...
	$reason    = (isset($_POST['reason']) && preg_match("/^[a-z0-9_]+([\\s]{1}[a-z0-9_]|[a-z0-9_])+$/i", $_POST['reason'])) ? $db->escape(strip_tags(stripslashes($_POST['reason']))) : '';
	$q         = $db->query("SELECT `gangNAME`, `gangPRESIDENT`, `gangVICEPRES`
                     FROM `gangs`
                     WHERE `gangID` = $gang");
	if ($db->num_rows($q) == 0) {
		$db->free_result($q);
	...
	if ($gang && $reason && $president && $vicepres) {
		staff_csrf_stdverify('staff_gangs_edit_staff', "staff_gangs.php?action=gedit_staff&amp;gang={$gang}");
		$db->query("UPDATE `gangs`
...
function admin_gang_edit_capacity()
{
	...
	$reason   = (isset($_POST['reason']) && preg_match("/^[a-z0-9_]+([\\s]{1}[a-z0-9_]|[a-z0-9_])+$/i", $_POST['reason'])) ? $db->escape(strip_tags(stripslashes($_POST['reason']))) : '';
	$q        = $db->query("SELECT `gangNAME`, `gangCAPACITY`
                     FROM `gangs`
                     WHERE `gangID` = $gang");
	if ($db->num_rows($q) == 0) {
		$db->free_result($q);
	...
	if ($gang && $reason && $capacity) {
		staff_csrf_stdverify('staff_gangs_edit_capacity', "staff_gangs.php?action=gedit_capacity&amp;gang={$gang}");
		$db->query("UPDATE `gangs`
...
function admin_gang_edit_crime()
{
	...
	$reason = (isset($_POST['reason']) && preg_match("/^[a-z0-9_]+([\\s]{1}[a-z0-9_]|[a-z0-9_])+$/i", $_POST['reason'])) ? $db->escape(strip_tags(stripslashes($_POST['reason']))) : '';
	$q      = $db->query("SELECT `gangNAME`, `gangCRIME`, `gangCHOURS`
                     FROM `gangs`
                     WHERE `gangID` = $gang");
	if ($db->num_rows($q) == 0) {
		$db->free_result($q);
	...
	if ($gang && $reason && $crime && $chours) {
		staff_csrf_stdverify('staff_gangs_edit_crime', "staff_gangs.php?action=gedit_crime&amp;gang={$gang}");
		$db->query("UPDATE `gangs`
...
function admin_gang_edit_ament()
{
	...
	$_POST['gangAMENT'] = (isset($_POST['gangAMENT'])) ? $db->escape(strip_tags(stripslashes($_POST['gangAMENT']))) : '';
	$q                  = $db->query("SELECT `gangNAME`, `gangAMENT`
                     FROM `gangs`
                     WHERE `gangID` = $gang");
	if ($db->num_rows($q) == 0) {
		$db->free_result($q);
	...
	if ($gang && $_POST['gangAMENT']) {
		staff_csrf_stdverify('staff_gangs_edit_ament', "staff_gangs.php?action=gedit_ament&amp;gang={$gang}");
		$db->query("UPDATE `gangs`
