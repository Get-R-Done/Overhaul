...
function error_critical($human_error, $debug_error, $action, $context = array())
{
	...
	if (DEBUG) {
		echo 'A critical error has occurred, and page execution has stopped. ' . 'Below are the details:<br />' . $debug_error . '<br /><br />' . '<strong>Action taken:</strong> ' . $action . '<br /><br />';
		...
		echo 'A critical error has occurred, and this page cannot be displayed. ' . 'Please try again later.';
...
function error_php($errno, $errstr, $errfile = '', $errline = 0, $errcontext = array())
{
	...
	if ($errno == E_WARNING) {
		...
		error_critical('', '<strong>Engine Error:</strong> ' . $errstr . ' (' . $errno . ')', 'Line executed: ' . $errfile . ':' . $errline, $errcontext);
	} else if ($errno == E_USER_WARNING) {
		error_critical('', '<strong>Engine Warning:</strong> ' . $errstr . ' (' . $errno . ')', 'Line executed: ' . $errfile . ':' . $errline, $errcontext);
	} else {
