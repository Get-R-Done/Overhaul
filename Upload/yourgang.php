...
{
	if (!isset($_POST['verf']) || !verify_csrf_code($formid, stripslashes($_POST['verf']))) {
		csrf_error($goBackTo);
	}
}
if (!$ir['gang']) {
	...
	<h3><u>Your Gang - {$gangdata['gangNAME']}</u></h3>
   	";
	$wq = $db->query("SELECT COUNT(`warID`)
                     FROM `gangwars`
	...
	if ($db->fetch_single($wq) > 0) {
		...
		<h3>
			...
			<span style='color: red;'>Your gang is currently in " . $db->fetch_single($wq) . " war(s).</span>
            </a>
        </h3>
   		";
	}
	...
	switch ($_GET['action']) {
		case "idx":gang_index();
			break;
		case "summary":gang_summary();break;
		case "members":gang_memberlist();break;
		case "kick":gang_staff_kick();break;
		case "forums":gang_forums();break;
		case "donate":gang_donate();break;
		case "donate2":gang_donate2();break;
		case "warview":gang_warview();break;
		case "staff":gang_staff();break;
		case "leave":gang_leave();break;
		case "atklogs":gang_atklogs();break;
		case "crimes":gang_crimes();break;
		default:gang_index();break;
	}
}
function gang_index()
{
	...
	<br />
   	";
	$q = $db->query("SELECT `gevTIME`, `gevTEXT`
                     FROM `gangevents`
	...
	while ($r = $db->fetch_row($q)) {
		echo "
		<tr>
			<td>" . date('F j Y, g:i:s a', $r['gevTIME']) . "</td>
			<td>{$r['gevTEXT']}</td>
		</tr>
   		";
	}
...
function gang_memberlist()
{
    ...
    <table cellspacing='1' class='table'>
       ...
       ";
	$q    = $db->query("SELECT `userid`, `username`, `daysingang`, `level`
                     FROM `users`
                     WHERE `gang` = {$gangdata['gangID']}
	...
	if ($gangdata['gangPRESIDENT'] == $userid || $gangdata['gangVICEPRES'] == $userid) {
		...
		if ($who == $gangdata['gangPRESIDENT']) {
			...
			if ($db->num_rows($q) > 0) {
				...
				$their_event = "You were kicked out of {$gangdata['gangNAME']} by " . "<a href='viewuser.php?u={$userid}'>" . $d_oname . "</a>";
				event_add($who, $their_event, $c);
				$gang_event = $db->escape("<a href='viewuser.php?u={$who}'>" . $d_username . "</a> was kicked out of the gang by " . "<a href='viewuser.php?u={$userid}'>" . $d_oname . "</a>");
				$db->query("INSERT INTO `gangevents`
                         VALUES(NULL, {$gangdata['gangID']}, " . time() . ", '{$gang_event}');");
			} else {
...
function gang_donate2()
{
	...
	$_POST['money']    = (isset($_POST['money']) && is_numeric($_POST['money'])) ? abs(intval($_POST['money'])) : 0;
	$_POST['crystals'] = (isset($_POST['crystals']) && is_numeric($_POST['crystals'])) ? abs(intval($_POST['crystals'])) : 0;
	...
	if ($_POST['money'] > $ir['money']) {
		...
		$gang_event = $db->escape("<a href='viewuser.php?u={$userid}'>" . $my_name . '</a>' . ' donated ' . money_formatter($_POST['money']) . ' and/or ' . number_format($_POST['crystals']) . ' crystals to the Gang.');
		$db->query("INSERT INTO `gangevents`
                 VALUES(NULL, {$gangdata['gangID']}, " . time() . ", '{$gang_event}')");
		echo "You donated " . money_formatter($_POST['money']) . " and/or {$_POST['crystals']} crystals to the Gang.<br />
...
function gang_leave()
{
	...
	if (isset($_POST['submit']) && $_POST['submit'] == 'Yes, leave!') {
		...
		$db->query("UPDATE `users`
        		 SET `gang` = 0, `daysingang` = 0
        		 WHERE `userid` = {$userid}");
		$gang_event = $db->escape("<a href='viewuser.php?u={$userid}'>" . htmlentities($ir['username'], ENT_QUOTES, 'ISO-8859-1') . '</a> left the Gang.');
		$db->query("INSERT INTO `gangevents`
                                VALUES(NULL, {$ir['gang']}, " . time() . ", '{$gang_event}')");
...
function gang_warview()
{
	...
	while ($r = $db->fetch_row($wq)) {
		...
		$ggq  = $db->query("SELECT `gangID`, gangNAME`
         				 FROM `gangs`
         				 WHERE `gangID` = " . $r[$f]);
...
{
	global $db, $ir, $c, $userid, $gangdata;
	$atks = $db->query("SELECT `a`.*, `u1`.`username` AS `attackern`,
...
function gang_staff()
{
	...
	if ($gangdata['gangPRESIDENT'] == $userid || $gangdata['gangVICEPRES'] == $userid) {
		...
		switch ($_GET['act2']) {
			case "idx":gang_staff_idx();break;
			case "apps":gang_staff_apps();break;
			case "vault":gang_staff_vault();break;
			case "vicepres":gang_staff_vicepres();break;
			case "pres":gang_staff_pres();break;
			case "upgrade":gang_staff_upgrades();break;
			case "declare":gang_staff_wardeclare();break;
			case "surrender":gang_staff_surrender();break;
			case "viewsurrenders":gang_staff_viewsurrenders();break;
			case "crimes":gang_staff_orgcrimes();break;
			case "massmailer":gang_staff_massmailer();break;
			case "desc":gang_staff_desc();break;
			case "ament":gang_staff_ament();break;
			case "name":gang_staff_name();break;
			case "tag":gang_staff_tag();break;
			case "masspayment":gang_staff_masspayment();break;
			default:gang_staff_idx();break;
		}
	} else {
...
function gang_staff_apps()
{
	...
	if (!empty($_POST['app']) && !empty($what)) {
		...
		$aq = $db->query("SELECT `appUSER`, `username`
		...
		if ($db->num_rows($aq) > 0) {
			...
			if ($what == 'decline') {
				...
				$gang_event = $db->escape("<a href='viewuser.php?u={$userid}'>" . $ir['username'] . '</a> has declined ' . "<a href='viewuser.php?u={$appdata['appUSER']}'>" . $appdata['username'] . '</a>\'s application to join the Gang.');
				$db->query("INSERT INTO `gangevents`
                         VALUES (NULL, {$gangdata['gangID']}, " . time() . ", '{$gang_event}')");
				echo "
				...
				$gang_event = $db->escape("<a href='viewuser.php?u={$userid}'>" . $ir['username'] . '</a> has accepted ' . "<a href='viewuser.php?u={$appdata['appUSER']}'>" . $appdata['username'] . '</a>\'s application to join the Gang.');
				$db->query("INSERT INTO `gangevents`
                         VALUES (NULL, {$gangdata['gangID']}, " . time() . ", '{$gang_event}')");
				$db->query("UPDATE `users`
        ...
        <table width='85%' cellspacing='1' class='table'>
   		...
   		";
		$q    = $db->query("SELECT `appTEXT`, `userid`, `username`, `level`,
                         `money`, `appID`
                         FROM `applications` AS `a`
                         INNER JOIN `users` AS `u`
                         ON `a`.`appUSER` = `u`.`userid`
		...
		while ($r = $db->fetch_row($q)) {
			$r['appTEXT'] = htmlentities($r['appTEXT'], ENT_QUOTES, 'ISO-8859-1', false);
			echo "
            <tr>
            	<td>
...
{
	global $db, $ir, $c, $userid, $gangdata;
	$_POST['who'] = (isset($_POST['who']) && is_numeric($_POST['who'])) ? abs(intval($_POST['who'])) : '';
	if (!empty($_POST['who'])) {
		...
		if ($_POST['crystals'] > $gangdata['gangCRYSTALS']) {
			...
			$dname = htmlentities($db->fetch_single($md), ENT_QUOTES, 'ISO-8859-1');
			$db->free_result($md);
			$money = $_POST['money'];
			$crys  = $_POST['crystals'];
			$db->query("UPDATE `users`
			...
			$db->query("UPDATE `gangs`
                     ...
                     WHERE `gangID` = {$gangdata['gangID']}");
			event_add($who, "You were given " . money_formatter($money) . " and/or $crys crystals from your Gang.", $c);
			$gang_event = $db->escape("<a href='viewuser.php?u=$who'>" . $dname . '</a> was given ' . money_formatter($money) . ' and/or ' . number_format($crys) . ' crystals from the Gang.');
			$db->query("INSERT INTO `gangevents`
                     VALUES(NULL, {$gangdata['gangID']}, " . time() . ",'{$gang_event}')");
			echo "<a href='viewuser.php?u=$who'>{$dname}</a> was given " . money_formatter($money) . ' and/or ' . number_format($crys) . ' crystals from the Gang.';
		}
	} else {
		$csrf = request_csrf_html('yourgang_staff_vault');
		echo "The vault has " . money_formatter($gangdata['gangMONEY']) . " and {$gangdata['gangCRYSTALS']} crystals.<br />
		...
		$q = $db->query("SELECT `userid` , `username`
                         FROM `users`
                         WHERE `gang` = {$gangdata['gangID']}");
		while ($r = $db->fetch_row($q)) {
...
function gang_staff_vicepres()
{
	...
	if (isset($_POST['subm'])) {
		...
		$_POST['vp'] = (isset($_POST['vp']) && is_numeric($_POST['vp'])) ? abs(intval($_POST['vp'])) : 0;
		$q           = $db->query("SELECT `userid`, `username`
                         FROM `users`
...
function gang_staff_wardeclare()
{
	...
	if (isset($_POST['subm'])) {
		...
		$_POST['gang'] = (isset($_POST['gang']) && is_numeric($_POST['gang'])) ? abs(intval($_POST['gang'])) : 0;
		...
		$data_q = $db->query("SELECT `gangNAME`
         				 FROM `gangs`
		...
		$db->free_result($data_q);
		$db->query("INSERT INTO `gangwars`
                 VALUES(NULL, {$gangdata['gangID']}, {$_POST['gang']}, " . time() . ")");
		$event   = $db->escape("<a href='gangs.php?action=view&amp;ID={$gangdata['gangID']}'>" . $gangdata['gangNAME'] . '</a> declared war on ' . "<a href='gangs.php?action=view&amp;ID={$_POST['gang']}'>" . $them . '</a>');
		$ev_time = time();
		$db->query("INSERT INTO `gangevents`
		...
		$q = $db->query("SELECT `gangID`, `gangNAME`
         				 FROM `gangs`
         				 WHERE `gangID` != {$gangdata['gangID']}");
		while ($r = $db->fetch_row($q)) {
...
function gang_staff_surrender()
{
	...
	if (!isset($_POST['subm'])) {
		...
		$_POST['war'] = (isset($_POST['war']) && is_numeric($_POST['war'])) ? abs(intval($_POST['war'])) : 0;
		$e_msg        = $db->escape(htmlentities(stripslashes($_POST['msg']), ENT_QUOTES, 'ISO-8859-1'));
		$wq           = $db->query("SELECT *
                         FROM gangwars
		...
		$db->query("INSERT INTO `surrenders`
		...
		$them = $db->fetch_single($ggq);
		$db->free_result($ggq);
		$event  = $db->escape("<a href='gangs.php?action=view&amp;ID={$gangdata['gangID']}'>" . $gangdata['gangNAME'] . '</a> have asked to surrender the war against ' . "<a href='gangs.php?action=view&amp;ID={$r[$f]}'>" . $them . '</a>');
		$e_time = time();
		$db->query("INSERT INTO `gangevents`
...
function gang_staff_viewsurrenders()
{
	...
	if (!isset($_POST['subm'])) {
		...
		csrf_stdverify('yourgang_staff_acceptsurrender', 'staff&amp;act2=viewsurrenders');
		$_POST['sur'] = (isset($_POST['sur']) && is_numeric($_POST['sur'])) ? abs(intval($_POST['sur'])) : 0;
		$q            = $db->query("SELECT `w`.*
                         FROM `surrenders` AS `s`
                         INNER JOIN `gangwars` AS `w`
                         ON `s`.`surWAR` = `w`.`warID`
		...
		$db->query("DELETE FROM `surrenders`
         		 WHERE `surWAR` = {$warID}");
		...
		$them = $db->fetch_single($ggq);
		$db->free_result($ggq);
		$event   = $db->escape("<a href='gangs.php?action=view&amp;ID={$gangdata['gangID']}'>" . $gangdata['gangNAME'] . '</a> have accepted the surrender from ' . "<a href='gangs.php?action=view&amp;ID={$r[$f]}'>" . $them . '</a>, the war is over!');
		$ev_time = time();
		$db->query("INSERT INTO `gangevents`
                 VALUES(NULL, {$gangdata['gangID']}, {$ev_time}, '$event'),
...
function gang_staff_orgcrimes()
{
	...
	if ($_POST['crime']) {
		...
		if ($gangdata['gangCRIME'] != 0) {
			...
			$crime_eq = $db->query("SELECT COUNT(`ocID`)
                             FROM `orgcrimes`
		...
		$db->free_result($cnt);
		$q = $db->query("SELECT `ocID`, `ocNAME`, `ocUSERS`
                         FROM `orgcrimes`
                         WHERE `ocUSERS` <= $membs");
		if ($db->num_rows($q) > 0) {
...
function gang_staff_pres()
{
	...
	if ($gangdata['gangPRESIDENT'] == $userid) {
		if (isset($_POST['subm'])) {
			...
			$_POST['pres'] = (isset($_POST['pres']) && is_numeric($_POST['pres'])) ? abs(intval($_POST['pres'])) : 0;
			$q             = $db->query("SELECT `userid`, `username`
                             FROM `users`
...
function gang_staff_upgrades()
{
	...
	if (isset($_POST['membs'])) {
		...
		$_POST['membs'] = (isset($_POST['membs']) && is_numeric($_POST['membs'])) ? abs(intval($_POST['membs'])) : 0;
		if ($_POST['membs'] == 0) {
			...
			$db->query("UPDATE `gangs`
            		 ...
            		 WHERE `gangID` = {$gangdata['gangID']}");
			echo "You paid " . money_formatter($cost) . " to add {$_POST['membs']} capacity to your gang.";
		}
	} else {
		...
		<form action='yourgang.php?action=staff&amp;act2=upgrade' method='post'>
			...
			Each extra member slot costs " . money_formatter(100000) . ".<br />
...
{
	global $db, $ir, $c, $userid, $gangdata;
	$_POST['text'] = (isset($_POST['text']) && strlen($_POST['text']) < 500) ? $db->escape(htmlentities(stripslashes($_POST['text']), ENT_QUOTES, 'ISO-8859-1')) : '';
	if (!empty($_POST['text'])) {
		csrf_stdverify('yourgang_staff_massmailer', 'staff&amp;act2=massmailer');
		$subj      = 'This is a mass mail from your gang';
		$mass_time = time();
		$q         = $db->query("SELECT `username`, `userid`
                         FROM `users`
                         WHERE `gang` = {$gangdata['gangID']}");
		while ($r = $db->fetch_row($q)) {
...
{
	global $db, $ir, $c, $userid, $gangdata;
	$_POST['amt'] = (isset($_POST['amt']) && is_numeric($_POST['amt'])) ? abs(intval($_POST['amt'])) : 0;
	if ($_POST['amt']) {
		csrf_stdverify('yourgang_staff_masspayment', 'staff&amp;act2=masspayment');
		$q = $db->query("SELECT `userid`, `username`
                         FROM `users`
                         WHERE `gang` = {$gangdata['gangID']}");
		while ($r = $db->fetch_row($q)) {
			if ($gangdata['gangMONEY'] >= $_POST['amt']) {
				event_add($r['userid'], "You were given " . money_formatter($_POST['amt']) . " from your gang.", $c);
				$db->query("UPDATE `users`
		...
		$db->query("UPDATE `gangs`
                 ...
                 WHERE `gangID` = {$gangdata['gangID']}");
		$credit_evt = $db->escape("A mass payment of " . money_formatter($_POST['amt']) . " was sent to the members of the Gang.");
		$db->query("INSERT INTO `gangevents`
                 VALUES(NULL, {$gangdata['gangID']}, " . time() . ", '{$credit_evt}')");
...
function gang_staff_tag()
{
	...
	if ($gangdata['gangPRESIDENT'] == $userid) {
		if (isset($_POST['subm']) && isset($_POST['tag'])) {
			...
			$tag = $db->escape(htmlentities(stripslashes($_POST['tag']), ENT_QUOTES, 'ISO-8859-1'));
			$db->query("UPDATE `gangs`
                     SET `gangPREF` = '{$tag}'
                     WHERE `gangID` = {$gangdata['gangID']}");
