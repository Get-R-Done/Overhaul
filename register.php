...
$username = (isset($_POST['username']) && preg_match("/^[a-z0-9_]+([\\s]{1}[a-z0-9_]|[a-z0-9_])+$/i", $_POST['username']) && ((strlen($_POST['username']) < 32) && (strlen($_POST['username']) >= 3))) ? stripslashes($_POST['username']) : '';
if (!empty($username)) {
	if ($set['regcap_on']) {
	...
	if ($u_check > 0) {
		...
		if ($_POST['ref']) {
			...
			$db->query("INSERT INTO `referals`
                     VALUES(NULL, {$_POST['ref']}, $i, " . time() . ", '{$e_rip}', '$IP')");
		}
