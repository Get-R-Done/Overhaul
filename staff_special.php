...
function give_dp_submit()
{
	...
	$_POST['type'] = (isset($_POST['type']) && in_array($_POST['type'], array(1,2,3,4,5))) ? abs((int) $_POST['type']) : '';
...
function staff_list()
{
	...
	$staff = array();
	$q     = $db->query("SELECT `userid`, `laston`, `username`, `level`, `money`,
 				 	 `user_level`
 				 	 FROM `users`
	...
	foreach ($staff as $r) {
		if ($r['user_level'] == 2) {
    		...
    		<tr>
    			...
    			<td>$on</td>
    			<td>
    				" . staff_userlevel_innerform($r['userid'], 3, 'Secretary', $csrf) . "
    				&middot; " . staff_userlevel_innerform($r['userid'], 5, 'Assistant', $csrf) . "
    				&middot; " . staff_userlevel_innerform($r['userid'], 1, 'Member', $csrf) . "
    			</td>
    		</tr>
       		";
		}
	...
	foreach ($staff as $r) {
		if ($r['user_level'] == 3) {
    		...
    		<tr>
    			...
    			<td>$on</td>
    			<td>
    				" . staff_userlevel_innerform($r['userid'], 2, 'Admin', $csrf) . "
    				&middot; " . staff_userlevel_innerform($r['userid'], 5, 'Assistant', $csrf) . "
    				&middot; " . staff_userlevel_innerform($r['userid'], 1, 'Member', $csrf) . "
    			</td>
    		</tr>
	...
	foreach ($staff as $r) {
		if ($r['user_level'] == 5) {
    		...
    		<tr>
    			...
    			<td>
    				" . staff_userlevel_innerform($r['userid'], 2, 'Admin', $csrf) . "
    				&middot; " . staff_userlevel_innerform($r['userid'], 3, 'Secretary', $csrf) . "
    				&middot; " . staff_userlevel_innerform($r['userid'], 1, 'Member', $csrf) . "
    			</td>
    		</tr>
       		";
...
{
	global $db, $ir, $c, $h, $userid;
	staff_csrf_stdverify('staff_userlevel', 'staff_special.php?action=userlevelform');
	$_POST['level'] = (isset($_POST['level']) && in_array($_POST['level'], array(1,2,3,4,5))) ? abs(intval($_POST['level'])) : 0;
...
function massmailer()
{
	...
	$_POST['text']  = (isset($_POST['text'])) ? $db->escape(strip_tags(stripslashes($_POST['text']))) : '';
	$_POST['cat']   = (isset($_POST['cat']) && in_array($_POST['cat'], array(1,2,3))) ? $_POST['cat'] : '';
	$_POST['level'] = (isset($_POST['level']) && in_array($_POST['level'], array(1,2,3,5))) ? abs((int) $_POST['level']) : '';
	if (!empty($_POST['text']) && (!empty($_POST['cat']) || empty($_POST['level']))) {
		...
		staff_csrf_stdverify('staff_massmailer', 'staff_special.php?action=massmailer');
