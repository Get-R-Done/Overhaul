...
if ($_GET['delete'] > 0) {
	$d_c = $db->query("SELECT COUNT(`evUSER`)
                     FROM `events`
...
if (isset($_GET['delall2']) && $_GET['delall2']) {
	if (!isset($_GET['verf']) || !verify_csrf_code('events_delall', stripslashes($_GET['verf']))) {
		echo '<h3>Error</h3><hr />
	...
	$am = $db->fetch_single($db->query("SELECT COUNT(`evID`)
                             FROM `events`
                             WHERE `evUSER` = $userid"));
	if ($am == 0) {
...
<hr />
   ";
$q = $db->query("SELECT `evTIME`, `evREAD`, `evTEXT`, `evID`
                FROM `events`
                WHERE `evUSER` = $userid
