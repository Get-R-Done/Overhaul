...
$_GET['u'] = (isset($_GET['u']) && is_numeric($_GET['u'])) ? abs(intval($_GET['u'])) : '';
if (!$_GET['u']) {
	echo 'Invalid use of file';
} else {
	$q = $db->query("SELECT `userid`, `user_level`, `laston`, `last_login`,
	...
	if ($db->num_rows($q) == 0) {
		...
		$u_duties    = ($r['user_level'] > 1) ? 'Duties: ' . $r['duties'] . '<br />' : '';
		$user_name   = ($r['donatordays']) ? '<span style="color:red; font-weight:bold;">' . $r['username'] . '</span> [' . $r['userid'] . '] <img src="donator.gif" alt="Donator: ' . $r['donatordays'] . ' Days Left" title="Donator: ' . $r['donatordays'] . ' Days Left" />' : $r['username'] . ' [' . $r['userid'] . ']';
		$on          = ($r['laston'] >= $_SERVER['REQUEST_TIME'] - 15 * 60) ? '<font color="green"><b>Online</b></font>' : '<font color="red"><b>Offline</b></font>';
		$ref_q       = $db->query("SELECT COUNT(`refID`)
                         FROM `referals`
    	...
    	<tr>
    		...
    		<td>
   		";
		echo ($r['display_pic']) ? '<img src="' . $r['display_pic'] . '" width="150px" height="150px" alt="User Display Pic" title="User Display Pic" />' : 'No Image';
		...
		<tr>
   		...
   		";
		echo ($r['gang']) ? 'Gang: <a href="gangs.php?action=view&ID=' . $r['gang'] . '">' . $r['gangNAME'] . '</a>' : '';
		if ($r['fedjail']) {
			echo "
            <br />
		...
		if (in_array($ir['user_level'], array(2,3,5))) {
            ...
            <form action='staffnotes.php' method='post'>
            	Staff Notes:
            	<br />
            	<textarea rows=7 cols=40 name='staffnotes'>" . htmlentities($r['staffnotes'], ENT_QUOTES, 'ISO-8859-1') . "</textarea>
            	<br />
