...
{
	global $c;
	$_GET['XID'] = (isset($_GET['XID']) && is_numeric($_GET['XID'])) ? abs(intval($_GET['XID'])) : 0;
...
function fed_user_submit()
{
	...
	$re = $db->query("UPDATE `users`
                     SET `fedjail` = 1
                     WHERE `userid` = {$_POST['user']}");
	if ($db->affected_rows() > 0) {
		$db->query("INSERT INTO `fedjail`
                 VALUES(NULL, {$_POST['user']}, {$_POST['days']}, $userid, '" . $_POST['reason'] . "')");
	}
	$db->query("INSERT INTO `jaillogs`
             VALUES(NULL, $userid, {$_POST['user']}, {$_POST['days']},
             '{$_POST['reason']}', " . time() . ")");
	stafflog_add('Fedded ID ' . $_POST['user'] . ' for ' . $_POST['days'] . ', reason: ' . $_POST['reason']);
	echo 'User jailed.<br />
...
{
	global $db, $ir, $c, $h, $userid;
	staff_csrf_stdverify('staff_mailbanuser', 'staff_punit.php?action=mailform');
	$_POST['user']   = (isset($_POST['user']) && is_numeric($_POST['user'])) ? abs(intval($_POST['user'])) : '';
	$_POST['reason'] = (isset($_POST['reason'])) ? $db->escape(strip_tags(stripslashes($_POST['reason']))) : '';
	$_POST['days']   = (isset($_POST['days']) && is_numeric($_POST['days'])) ? abs(intval($_POST['days'])) : '';
	if (empty($_POST['user']) || empty($_POST['reason']) || empty($_POST['days'])) {
...
{
	global $db, $ir, $c, $h, $userid;
	staff_csrf_stdverify('staff_forumbanuser', 'staff_punit.php?action=forumform');
	$_POST['user']   = (isset($_POST['user']) && is_numeric($_POST['user'])) ? abs(intval($_POST['user'])) : '';
	$_POST['reason'] = (isset($_POST['reason'])) ? $db->escape(strip_tags(stripslashes($_POST['reason']))) : '';
	$_POST['days']   = (isset($_POST['days']) && is_numeric($_POST['days'])) ? abs(intval($_POST['days'])) : '';
	if (empty($_POST['user']) || empty($_POST['reason']) || empty($_POST['days'])) {
...
function unfed_user_submit()
{
	...
	$q = $db->query('SELECT COUNT(`userid`)
                     FROM `users`
                     WHERE `userid` = ' . $_POST['user']);
	...
	$db->query("DELETE FROM `fedjail`
     		 WHERE `fed_userid` = {$_POST['user']}");
	$db->query("INSERT INTO `unjaillogs`
...
function unmail_user_form()
{
    ...
    <form action='staff_punit.php?action=unmailsub' method='post'>
    	User: " . mailb_user_dropdown(NULL, 'user') . "<br />
        {$csrf}
...
{
	global $db, $ir, $c, $h, $userid;
	staff_csrf_stdverify('staff_unmailbanuser', 'staff_punit.php?action=unmailform');
	$_POST['user'] = (isset($_POST['user']) && is_numeric($_POST['user'])) ? abs(intval($_POST['user'])) : '';
	if (empty($_POST['user'])) {
		echo 'You need to fill in all the fields.<br />
...
{
	global $db, $ir, $c, $h, $userid;
	staff_csrf_stdverify('staff_unforumbanuser', 'staff_punit.php?action=unforumform');
	$_POST['user'] = (isset($_POST['user']) && is_numeric($_POST['user'])) ? abs(intval($_POST['user'])) : '';
	if (empty($_POST['user'])) {
		echo 'You need to fill in all the fields.<br />
...
function ip_search_submit()
{
    ...
    <table width='75%' class='table' cellpadding='1' cellspacing='1'>
       ...
       ";
	$q   = $db->query("SELECT `userid`, `username`, `level`, `money`
                     FROM `users`
                     WHERE `lastip` = '" . $db->escape(stripslashes($_POST['ip'])) . "'");
	$ids = array();
