...
Money - " . money_formatter($set['ct_moneypercrys']) . " per crystal</a><br />";
} else {
	if ($_GET['spend'] == 'refill') {
		...
		if (!isset($_POST['verf']) || !verify_csrf_code('ctemple_iq', stripslashes($_POST['verf']))) {
			csrf_error('IQ');
		}
		$_POST['crystals'] = isset($_POST['crystals']) ? (int) $_POST['crystals'] : 0;
		...
		if (!isset($_POST['verf']) || !verify_csrf_code('ctemple_money', stripslashes($_POST['verf']))) {
			csrf_error('money');
		}
		$_POST['crystals'] = isset($_POST['crystals']) ? (int) $_POST['crystals'] : 0;
		if ($_POST['crystals'] <= 0 || $_POST['crystals'] > $ir['crystals']) {
			...
			$db->query("UPDATE `users`
                     ...
                     `money` = `money` + $iqgain
            		 WHERE `userid` = $userid");
			echo "You traded {$_POST['crystals']} crystals for " . money_formatter($iqgain) . ".";
		}
	}
}
$h->endpage();
