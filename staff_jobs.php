...
function newjob()
{
	...
	$price              = (isset($_POST['price']) && is_numeric($_POST['price'])) ? abs(intval($_POST['price'])) : '';
	$will               = (isset($_POST['will']) && is_numeric($_POST['will'])) ? abs(intval($_POST['will'])) : '';
	$_POST['jNAME']     = (isset($_POST['jNAME']) && preg_match("/^[a-z0-9_]+([\\s]{1}[a-z0-9_]|[a-z0-9_])+$/i", $_POST['jNAME'])) ? $db->escape(strip_tags(stripslashes($_POST['jNAME']))) : '';
	$_POST['jDESC']     = (isset($_POST['jDESC'])) ? $db->escape(strip_tags(stripslashes($_POST['jDESC']))) : '';
	$_POST['jOWNER']    = (isset($_POST['jOWNER']) && preg_match("/^[a-z0-9_]+([\\s]{1}[a-z0-9_]|[a-z0-9_])+$/i", $_POST['jOWNER'])) ? $db->escape(strip_tags(stripslashes($_POST['jOWNER']))) : '';
	$_POST['jrNAME']    = (isset($_POST['jrNAME']) && preg_match("/^[a-z0-9_]+([\\s]{1}[a-z0-9_]|[a-z0-9_])+$/i", $_POST['jrNAME'])) ? $db->escape(strip_tags(stripslashes($_POST['jrNAME']))) : '';
	$_POST['jrPAY']     = (isset($_POST['jrPAY']) && is_numeric($_POST['jrPAY'])) ? abs(intval($_POST['jrPAY'])) : '';
	$_POST['jrSTRG']    = (isset($_POST['jrSTRG']) && is_numeric($_POST['jrSTRG'])) ? abs(intval($_POST['jrSTRG'])) : 0;
	$_POST['jrLABOURG'] = (isset($_POST['jrLABOURG']) && is_numeric($_POST['jrLABOURG'])) ? abs(intval($_POST['jrLABOURG'])) : 0;
	$_POST['jrIQG']     = (isset($_POST['jrIQG']) && is_numeric($_POST['jrIQG'])) ? abs(intval($_POST['jrIQG'])) : 0;
	$_POST['jrSTRN']    = (isset($_POST['jrSTRN']) && is_numeric($_POST['jrSTRN'])) ? abs(intval($_POST['jrSTRN'])) : 0;
	$_POST['jrLABOURN'] = (isset($_POST['jrLABOURN']) && is_numeric($_POST['jrLABOURN'])) ? abs(intval($_POST['jrLABOURN'])) : 0;
	$_POST['jrIQN']     = (isset($_POST['jrIQN']) && is_numeric($_POST['jrIQN'])) ? abs(intval($_POST['jrIQN'])) : 0;
	if (!empty($_POST['jNAME']) && !empty($_POST['jDESC']) && !empty($_POST['jOWNER']) && !empty($_POST['jrNAME']) && !empty($_POST['jrPAY']) && !empty($_POST['jrSTRN']) && !empty($_POST['jrLABOURN']) && !empty($_POST['jrIQN'])) {
...
function jobedit()
{
	...
	$_POST['jDESC']  = (isset($_POST['jDESC'])) ? $db->escape(strip_tags(stripslashes($_POST['jDESC']))) : '';
	$_POST['jOWNER'] = (isset($_POST['jOWNER']) && preg_match("/^[a-z0-9_]+([\\s]{1}[a-z0-9_]|[a-z0-9_])+$/i", $_POST['jOWNER'])) ? $db->escape(strip_tags(stripslashes($_POST['jOWNER']))) : '';
	$_POST['jFIRST'] = (isset($_POST['jFIRST']) && is_numeric($_POST['jFIRST'])) ? abs(intval($_POST['jFIRST'])) : '';
	$_POST['jID']    = (isset($_POST['jID']) && is_numeric($_POST['jID'])) ? abs(intval($_POST['jID'])) : '';
	if (!empty($_POST['jID']) && !empty($_POST['jFIRST']) && !empty($_POST['jOWNER']) && !empty($_POST['jDESC']) && !empty($_POST['jNAME'])) {
		staff_csrf_stdverify('staff_editjob2', 'staff_jobs.php?action=jobedit');
		$q = $db->query("SELECT COUNT(`jID`)
                         FROM `jobs`
                         WHERE `jID` = {$_POST['jID']}");
		...
		$q = $db->query("SELECT `jFIRST`, `jOWNER`, `jDESC`, `jNAME`
                         FROM `jobs`
                         WHERE `jID` = {$_POST['jID']}");
		if ($db->num_rows($q) == 0) {
			$db->free_result($q);
        ...
        <form action='staff_jobs.php?action=jobedit' method='post'>
        	...
        	<br />
        	<b>First Job Rank:</b> " . jobrank_dropdown(NULL, 'jFIRST', $r['jFIRST']) . "
        	<br />
        	{$csrf}
...
function newjobrank()
{
	global $db, $ir, $userid, $h;
	$_POST['jrNAME']    = (isset($_POST['jrNAME']) && preg_match("/^[a-z0-9_]+([\\s]{1}[a-z0-9_]|[a-z0-9_])+$/i", $_POST['jrNAME'])) ? $db->escape(strip_tags(stripslashes($_POST['jrNAME']))) : '';
	$_POST['jrJOB']     = (isset($_POST['jrJOB']) && is_numeric($_POST['jrJOB'])) ? abs(intval($_POST['jrJOB'])) : '';
	$_POST['jrPAY']     = (isset($_POST['jrPAY']) && is_numeric($_POST['jrPAY'])) ? abs(intval($_POST['jrPAY'])) : '';
	$_POST['jrSTRG']    = (isset($_POST['jrSTRG']) && is_numeric($_POST['jrSTRG'])) ? abs(intval($_POST['jrSTRG'])) : 0;
	$_POST['jrLABOURG'] = (isset($_POST['jrLABOURG']) && is_numeric($_POST['jrLABOURG'])) ? abs(intval($_POST['jrLABOURG'])) : 0;
	$_POST['jrIQG']     = (isset($_POST['jrIQG']) && is_numeric($_POST['jrIQG'])) ? abs(intval($_POST['jrIQG'])) : 0;
	...
	$_POST['jrLABOURN'] = (isset($_POST['jrLABOURN']) && is_numeric($_POST['jrLABOURN'])) ? abs(intval($_POST['jrLABOURN'])) : 0;
	$_POST['jrIQN']     = (isset($_POST['jrIQN']) && is_numeric($_POST['jrIQN'])) ? abs(intval($_POST['jrIQN'])) : 0;
	if (!empty($_POST['jrNAME']) && !empty($_POST['jrJOB']) && !empty($_POST['jrPAY']) && !empty($_POST['jrSTRN']) && !empty($_POST['jrLABOURN']) && !empty($_POST['jrIQN'])) {
		staff_csrf_stdverify('staff_newjobrank', 'staff_jobs.php?action=newjobrank');
		$q = $db->query("SELECT COUNT(`jID`)
                         FROM `jobs`
...
{
	global $db, $ir, $userid, $h;
	$_POST['jrID']      = (isset($_POST['jrID']) && is_numeric($_POST['jrID'])) ? abs(intval($_POST['jrID'])) : '';
	$_POST['jrNAME']    = (isset($_POST['jrNAME']) && preg_match("/^[a-z0-9_]+([\\s]{1}[a-z0-9_]|[a-z0-9_])+$/i", $_POST['jrNAME'])) ? $db->escape(strip_tags(stripslashes($_POST['jrNAME']))) : '';
	$_POST['jrJOB']     = (isset($_POST['jrJOB']) && is_numeric($_POST['jrJOB'])) ? abs(intval($_POST['jrJOB'])) : '';
	$_POST['jrPAY']     = (isset($_POST['jrPAY']) && is_numeric($_POST['jrPAY'])) ? abs(intval($_POST['jrPAY'])) : '';
	$_POST['jrSTRG']    = (isset($_POST['jrSTRG']) && is_numeric($_POST['jrSTRG'])) ? abs(intval($_POST['jrSTRG'])) : 0;
	...
	$_POST['jrLABOURN'] = (isset($_POST['jrLABOURN']) && is_numeric($_POST['jrLABOURN'])) ? abs(intval($_POST['jrLABOURN'])) : 0;
	$_POST['jrIQN']     = (isset($_POST['jrIQN']) && is_numeric($_POST['jrIQN'])) ? abs(intval($_POST['jrIQN'])) : 0;
	if (!empty($_POST['jrID']) && !empty($_POST['jrNAME']) && !empty($_POST['jrJOB']) && !empty($_POST['jrPAY']) && !empty($_POST['jrSTRN']) && !empty($_POST['jrLABOURN']) && !empty($_POST['jrIQN'])) {
		staff_csrf_stdverify('staff_editjobrank2', 'staff_jobs.php?action=jobrankedit');
		$q = $db->query("SELECT COUNT(`jrID`)
                         FROM `jobranks`
		...
		staff_csrf_stdverify('staff_editjobrank1', 'staff_jobs.php?action=jobrankedit');
		$q = $db->query("SELECT `jrIQN`, `jrLABOURN`, `jrSTRN`, `jrIQG`,
...
function jobrankdele()
{
	...
	$_POST['jrID'] = (isset($_POST['jrID']) && is_numeric($_POST['jrID'])) ? abs(intval($_POST['jrID'])) : '';
	if (!empty($_POST['jrID'])) {
		staff_csrf_stdverify('staff_deljobrank', 'staff_jobs.php?action=jobrankdele');
		$q = $db->query("SELECT `jrJOB`
                         FROM `jobranks`
                         WHERE `jrID` = {$_POST['jrID']}");
		if ($db->num_rows($q) == 0) {
			$db->free_result($q);
		...
		$db->free_result($q);
		$db->query("DELETE FROM `jobranks`
         		 WHERE `jrID` = {$_POST['jrID']}");
		echo "Job rank successfully deleted!";
		$q = $db->query("SELECT `jNAME`
                         FROM `jobs`
