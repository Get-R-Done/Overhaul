...
if (!((int) $_GET['ID'])) {
	...
	$it = $db->query("SELECT `bankmoney`, `lastip`, `username`
                     FROM `users`
	...
	if ($er['bankmoney'] == -1 || $ir['bankmoney'] == -1) {
		die("Sorry,you or the person you are sending to does not have a bank account.");
	}
	if ((int) $_POST['xfer']) {
		if (!isset($_POST['verf']) || !verify_csrf_code("sendbank_{$_GET['ID']}", stripslashes($_POST['verf']))) {
			echo '<h3>Error</h3><hr />
			...
			event_add($_GET['ID'], "You received " . money_formatter($_POST['xfer']) . " into your bank account from {$ir['username']}.", $c);
		...
		<br />You have <b>" . money_formatter($ir['bankmoney']) . "</b> you can send.
