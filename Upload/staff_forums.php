...
switch ($_GET['action']) {
	...
	case "editforum":editforum();break;
	case "delforum":delforum();break;
...
function addforum()
{
	...
	if ($auth && $desc && $name) {
		staff_csrf_stdverify('staff_addforum', 'staff_forums.php?action=addforum');
		$q = $db->query("SELECT COUNT(`ff_id`)
                         FROM `forum_forums`
                         WHERE `ff_name` = '{$name}'");
		...
		echo 'Forum ' . $name . ' added to the game.<br />&gt; <a href="staff.php">Goto Main</a>';
...
function editforum()
{
	...
	switch ($_POST['step']) {
		case "2":
			...
			staff_csrf_stdverify('staff_editforum2', 'staff_forums.php?action=editforum');
			$q = $db->query("SELECT COUNT(`ff_id`)
                         FROM `forum_forums`
                         WHERE `ff_name` = '{$name}'
                         AND `ff_id` != {$_POST['id']}");
			...
			$db->query("UPDATE `forum_forums`
                 ...
                 `ff_auth` = '$auth'
                 WHERE `ff_id` = {$_POST['id']}");
			echo 'Forum ' . $name . ' was edited successfully.<br />
		...
		case "1":
			...
			staff_csrf_stdverify('staff_editforum1', 'staff_forums.php?action=editforum');
			$q = $db->query("SELECT `ff_auth`, `ff_name`, `ff_desc`
                         FROM `forum_forums`
                         WHERE `ff_id` = {$_POST['id']}");
			if ($db->num_rows($q) == 0) {
				$db->free_result($q);
        ...
        <br />
        Authorization: <input type="radio" name="auth" value="public" ' . $check_p . ' /> Public <input type="radio" name="auth" value="staff" ' . $check_s . ' /> Staff
        <br />
        	' . $csrf . '
        ...
        <form action='staff_forums.php?action=editforum' method='post'>
        	...
        	Forum: " . forum2_dropdown(NULL, "id") . "<br />
            {$csrf}
        	<input type='submit' value='Edit Forum' />
        </form>
           ";
...
function delforum()
{
	...
	$_POST['forum2'] = (isset($_POST['forum2']) && is_numeric($_POST['forum2'])) ? abs(intval($_POST['forum2'])) : '';
	if ($_POST['forum'] && $_POST['forum2']) {
		staff_csrf_stdverify('staff_delforum', 'staff_forums.php?action=delforum');
		if ($_POST['forum'] == $_POST['forum2']) {
		...
		$q   = $db->query("SELECT `ff_name`
                         FROM `forum_forums`
                         WHERE `ff_id` = {$_POST['forum']}");
		$old = $db->fetch_single($q);
		$db->free_result($q);
		$db->query("DELETE FROM `forum_forums`
                 WHERE `ff_id` = {$_POST['forum']}");
		echo 'Forum ' . $old . ' deleted.<br />
        &gt; <a href="staff.php">Goto Main</a>';
