...
if (isset($_POST['bet']) && is_numeric($_POST['bet'])) {
	...
	if ($_POST['bet'] > $ir['money']) {
		...
		<a href='roulette.php?tresde=$tresder'>&gt; Back</a>");
	} else if ($_POST['number'] > 36 or $_POST['number'] < 0 or $_POST['bet'] < 0) {
	...
	if ($slot[1] == $_POST['number']) {
		$won  = $_POST['bet'] * 37;
		$gain = $_POST['bet'] * 36;
		echo "and won " . money_formatter($won) . " by matching the number you bet pocketing you " . money_formatter($gain) . " extra.";
	} else {
		$won  = 0;
		$gain = -$_POST['bet'];
		echo "and lost it.";
