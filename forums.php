...
class bbcode
{
	...
	function bbcode()
	{
		...
		$this->engine->cust_tag("/\[ul\](.+?)\[\/ul\]/is", "<table><tr><td align='left'><ul>\\1</ul></td></tr></table>");
		$this->engine->cust_tag("/\[ol\](.+?)\[\/ol\]/is", "<table><tr><td align='left'><ol>\\1</ol></td></tr></table>");
		$this->engine->cust_tag("/\[list\](.+?)\[\/list\]/is", "<table><tr><td align='left'><ul>\\1</ul></td></tr></table>");
		$this->engine->cust_tag("/\[olist\](.+?)\[\/olist\]/is", "<table><tr><td align='left'><ol>\\1</ol></td></tr></table>");
		...
		$this->engine->cust_tag("/\[left\](.+?)\[\/left\]/i", "<div align='left'>\\1</div>");
		$this->engine->cust_tag("/\[center\](.+?)\[\/center\]/i", "<div align='center'>\\1</div>");
		$this->engine->cust_tag("/\[right\](.+?)\[\/right\]/i", "<div align='right'>\\1</div>");
		$this->engine->cust_tag("/\[quote=(.+?)\]/i", "<div class='quotetop'>QUOTE (\\1)</div><div class='quotemain'>");
		$this->engine->cust_tag("/\[quote\]/i", "<div class='quotetop'>QUOTE</div><div class='quotemain'>");
		$this->engine->cust_tag("/\[\/quote\]/i", "</div>");
		$this->engine->cust_tag("/\[code\](.+?)\[\/code\]/i", "<div class='codetop'>CODE</div><div class='codemain'><code>\\1</code></div>");
		$this->engine->cust_tag("/\[codebox\](.+?)\[\/codebox\]/i", "<div class='codetop'>CODE</div><div class='codemain' style='height:200px;white-space:pre;overflow:auto'>\\1</div>");
		$this->engine->cust_tag("/\[img=(.+?)\]/ie", "check_image('\\1')");
		$this->engine->cust_tag("/\[img](.+?)\[\/img\]/ie", "check_image('\\1')");
		$this->engine->cust_tag("/&nbrlb;/", "<br />");
		$this->engine->cust_tag("/\[userbox\]([0-9]+)\[\/userbox\]/ie", "userBox(\\1)");
	...
	function bbcode_parse($html)
	{
		$html = str_ireplace(array(
			"javascript:","document.","onClick","onDblClick",
			"onLoad","onMouseOver","onBlur","onChange",
	...
	function quote_corrector($in)
	{
		...
		if ($quote_starts > $quotes) {
			...
			while ($quotes > $quote_starts) {
				$num = $quotes - 1;
				$in  = substr($in, 0, $poss[$num]) . ($poss[$num] + 8 >= strlen($in) ? "" : substr($in, $poss[$num] + 8));
				$quotes--;
			}
			return $in;
		} else {
...
{
	if (strpos($src, ".php") !== false || strpos($src, ".asp") !== false || strpos($src, ".aspx") !== false || strpos($src, ".htm") !== false) {
		return "invalid image";
	}
	if (strpos($src, ".gif") === false && strpos($src, ".jpg") === false && strpos($src, ".png") === false && strpos($src, ".jpeg") === false) {
		return "invalid image";
	}
...
if (isset($_GET['empty']) && $_GET['empty'] == 1 && isset($_GET['code']) && $_GET['code'] === 'kill' && isset($_SESSION['owner']) && $_SESSION['owner'] > 0) {
	emptyallforums();
}
switch ($_GET['act']) {
	...
	case 'newtopicform':newtopicform();break;
...
function idx()
{
	...
	$q = $db->query("SELECT `ff_lp_time`, `ff_id`, `ff_name`, `ff_desc`,
...
function viewforum()
{
	...
	$q = $db->query("SELECT `ff_auth`, `ff_owner`, `ff_name`
                     FROM `forum_forums`
	...
	$r = $db->fetch_row($q);
	$db->free_result($q);
	if (($r['ff_auth'] == 'gang' && $ir['gang'] != $r['ff_owner'] && $ir["user_level"] < 2) || ($r['ff_auth'] == 'staff' && $ir['user_level'] < 2)) {
		echo '
...
&gt; <a href="forums.php">Back</a>
   ';
	...
	$q = $db->query("SELECT `ft_start_time`, `ft_last_time`, `ft_pinned`,
...
function viewtopic()
{
	...
	$q = $db->query("SELECT `ft_forum_id`, `ft_name`, `ft_posts`, `ft_id`,
                    `ft_locked`
                     FROM `forum_topics`
	...
	$db->free_result($q);
	$q2 = $db->query("SELECT `ff_auth`, `ff_owner`, `ff_id`, `ff_name`
                    FROM `forum_forums`
	...
	$db->free_result($q2);
	if (($forum['ff_auth'] == 'gang' && $ir['gang'] != $forum['ff_owner'] && $ir["user_level"] < 2) || ($forum['ff_auth'] == 'staff' && $ir['user_level'] < 2)) {
		echo '
You have no permission to view this forum.<br />
&gt; <a href="forums.php">Back</a>
   ';
	...
	$q3 = $db->query("SELECT `fp_poster_name`, `fp_editor_time`,
	...
	while ($r = $db->fetch_row($q3)) {
		$qlink = "[<a href='forums.php?act=quote&amp;viewtopic={$_GET['viewtopic']}&amp;quotename=" . urlencode(htmlentities($r['fp_poster_name'], ENT_QUOTES, 'ISO-8859-1')) . "&amp;quotetext=" . urlencode(htmlentities($r['fp_text'], ENT_QUOTES, 'ISO-8859-1')) . "'>Quote Post</a>]";
		...
		if (!isset($precache[$r['fp_poster_id']])) {
			$membq = $db->query("SELECT `userid`, `posts`, `forums_avatar`,
...
{
	global $ir, $c, $userid, $h, $bbc, $db;
	$_GET['reply'] = (isset($_GET['reply']) && is_numeric($_GET['reply'])) ? abs(intval($_GET['reply'])) : '';
	if (empty($_GET['reply'])) {
		echo 'Something went wrong.<br />
        	  &gt; <a href="forums.php" title="Go Back">Go Back</a>';
		die($h->endpage());
	}
	$q = $db->query("SELECT `ft_forum_id`, `ft_locked`, `ft_name`, `ft_id`
                     FROM `forum_topics`
	...
	$topic = $db->fetch_row($q);
	$db->free_result($q);
	$q2 = $db->query("SELECT `ff_auth`, `ff_owner`, `ff_id`
                     FROM `forum_forums`
                     WHERE `ff_id` = {$topic['ft_forum_id']}");
	if ($db->num_rows($q2) == 0) {
		$db->free_result($q2);
	...
	$forum = $db->fetch_row($q2);
	$db->free_result($q2);
	if (($forum['ff_auth'] == 'gang' && $ir['gang'] != $forum['ff_owner']) || ($forum['ff_auth'] == 'staff' && $ir['user_level'] < 2)) {
		echo '
	...
	if (!isset($_POST['verf']) || !verify_csrf_code("forums_reply_{$topic['ft_id']}", stripslashes($_POST['verf']))) {
		echo '
	...
	if ($topic['ft_locked'] == 0) {
		...
		if ($ir['donatordays'] > 0) {
			$u = '<span style="color: red;">' . htmlentities($ir['username'], ENT_QUOTES, 'ISO-8859-1') . '</span>';
		}
		$u                   = $db->escape($u);
		$_POST['fp_subject'] = $db->escape(strip_tags(stripslashes($_POST['fp_subject'])));
...
function newtopicform()
{
	...
	$r = $db->fetch_row($q);
	$db->free_result($q);
	if (($r['ff_auth'] == 'gang' && $ir['gang'] != $r['ff_owner']) || ($r['ff_auth'] == 'staff' && $ir['user_level'] < 2)) {
		echo '
        You have no permission to view this forum.<br />
        &gt; <a href="forums.php">Back</a>
           ';
...
function newtopic()
{
	...
	$q = $db->query("SELECT `ff_auth`, `ff_owner`, `ff_id`
                     FROM `forum_forums`
                     WHERE `ff_id` = {$_GET['forum']}");
	if ($db->num_rows($q) == 0) {
		$db->free_result($q);
...
&gt; <a href="forums.php">Back</a>
   ';
	...
	$u                = $db->escape($u);
	$_POST['ft_name'] = $db->escape(strip_tags(stripslashes($_POST['ft_name'])));
	if ((strlen($_POST['ft_name']) > 255)) {
		echo 'You can only submit a max of 255 characters.<br />&gt; <a href="forums.php">Go Back</a>';
		die($h->endpage());
	}
	$_POST['ft_desc'] = $db->escape(strip_tags(stripslashes($_POST['ft_desc'])));
...
function quote()
{
	...
	$q = $db->query("SELECT `ft_forum_id`, `ft_name`, `ft_locked`, `ft_id`
                     FROM `forum_topics`
                     WHERE `ft_id` = {$_GET['viewtopic']}");
	if ($db->num_rows($q) == 0) {
		$db->free_result($q);
...
&gt; <a href="forums.php">Back</a>
   ';
    ...
    ";
	if ($topic['ft_locked'] == 0) {
		$_GET['quotename'] = htmlentities(strip_tags(stripslashes($_GET['quotename'])), ENT_QUOTES, 'ISO-8859-1');
		$_GET['quotetext'] = htmlentities(stripslashes($_GET['quotetext']), ENT_QUOTES, 'ISO-8859-1');
...
function edit()
{
	...
	$q = $db->query("SELECT `ft_forum_id`, `ft_name`, `ft_id`
                     FROM `forum_topics`
...
&gt; <a href="forums.php">Back</a>
   ';
	...
	$q3 = $db->query("SELECT `fp_poster_id`, `fp_subject`, `fp_text`
                     FROM `forum_posts`
...
function editsub()
{
	...
	$topic = $db->fetch_row($q);
	$db->free_result($q);
	$q2 = $db->query("SELECT `ff_auth`, `ff_owner`
                     FROM `forum_forums`
                     WHERE `ff_id` = {$topic['ft_forum_id']}");
	if ($db->num_rows($q2) == 0) {
		$db->free_result($q2);
...
&gt; <a href="forums.php">Back</a>
   ';
	...
	$q3 = $db->query("SELECT `fp_poster_id`
                     FROM `forum_posts`
                     WHERE `fp_id` = {$_GET['post']}");
	if ($db->num_rows($q3) == 0) {
		$db->free_result($q3);
...
function recache_forum($forum)
{
	...
	$q = $db->query("SELECT `fp_poster_name`, `fp_time`, `fp_poster_id`,
                     `ft_name`, `ft_id`
...
function recache_topic($topic)
{
	...
	$q = $db->query("SELECT `fp_poster_id`, `fp_poster_name`, `fp_time`
                     FROM `forum_posts`
...
function move()
{
	...
	$q = $db->query("SELECT `ft_name`, `ft_forum_id`
                     FROM `forum_topics`
                     WHERE `ft_id` = {$_GET['topic']}");
	if ($db->num_rows($q) == 0) {
		$db->free_result($q);
...
function lock()
{
	...
	$q = $db->query("SELECT `ft_name`,`ft_locked`,`ft_forum_id`
                     FROM `forum_topics`
                     WHERE `ft_id` = {$_GET['topic']}");
	if ($db->num_rows($q) == 0) {
		$db->free_result($q);
	...
	if ($r['ft_locked'] == 1) {
		$db->query("UPDATE `forum_topics`
                 SET `ft_locked` = 0
                 WHERE `ft_id` = {$_GET['topic']}");
		echo 'Topic unlocked.<br />&gt; <a href="forums.php?viewforum=' . $r['ft_forum_id'] . '" title="Go Back">Go Back</a>';
		stafflog_add("Unlocked Topic {$r['ft_name']}");
	} else {
		$db->query("UPDATE `forum_topics`
                 SET `ft_locked` = 1
                 WHERE `ft_id` = {$_GET['topic']}");
		echo 'Topic locked.<br />&gt; <a href="forums.php?viewforum=' . $r['ft_forum_id'] . '" title="Go Back">Go Back</a>';
...
function pin()
{
	...
	$q = $db->query("SELECT `ft_name`, `ft_pinned`, `ft_forum_id`
                     FROM `forum_topics`
                     WHERE `ft_id` = {$_GET['topic']}");
	if ($db->num_rows($q) == 0) {
		$db->free_result($q);
	...
	if ($r['ft_pinned'] == 1) {
		$db->query("UPDATE `forum_topics`
                 SET `ft_pinned` = 0
                 WHERE `ft_id` = {$_GET['topic']}");
		echo 'Topic unpinned.<br />&gt; <a href="forums.php?viewforum=' . $r['ft_forum_id'] . '" title="Go Back">Go Back</a>';
		stafflog_add("Unpinned Topic {$r['ft_name']}");
	} else {
		$db->query("UPDATE `forum_topics`
                 SET `ft_pinned` = 1
                 WHERE `ft_id` = {$_GET['topic']}");
		echo 'Topic pinned.<br />&gt; <a href="forums.php?viewforum=' . $r['ft_forum_id'] . '" title="Go Back">Go Back</a>';
...
function delepost()
{
	...
	$q3 = $db->query("SELECT `fp_topic_id`, `fp_poster_name`, `fp_id`,
	...
	$db->query("DELETE FROM `forum_posts`
...
function deletopic()
{
	...
	$q = $db->query("SELECT `ft_forum_id`, `ft_name`
                     FROM `forum_topics`
                     WHERE `ft_id` = {$_GET['topic']}");
	if ($db->num_rows($q) == 0) {
		$db->free_result($q);
	...
	$topic = $db->fetch_row($q);
	$db->free_result($q);
	$db->query("DELETE FROM `forum_topics`
    		    WHERE `ft_id` = {$_GET['topic']}");
	$db->query("DELETE FROM `forum_posts`
             WHERE `fp_topic_id` = {$_GET['topic']}");
