...
$hofheads       = array(
	'level','money','crystals','respect',
	'total','strength','agility','guard',
	'labour','iq'
);
$_GET['action'] = (isset($_GET['action']) && in_array($_GET['action'], $hofheads)) ? $_GET['action'] : 'level';
...
if (in_array($_GET['action'], $hofqone)) {
	$q = $db->query("SELECT `userid`, `laston`, `gender`, `donatordays`,
...
<table width='75%' cellspacing='1' class='table'>
   ...
   ";
	$q = $db->query("SELECT `gangID`, `gangNAME`, `gangRESPECT`
                     FROM `gangs`
