...
$_POST['name'] = (isset($_POST['name']) && is_string($_POST['name'])) ? stripslashes($_POST['name']) : '';
if (!$_POST['name']) {
	echo "Invalid use of file";
} elseif (!preg_match("/^[a-z0-9_]+([\\s]{1}[a-z0-9_]|[a-z0-9_])+$/i", $_POST['name'])) {
	...
	$q            = $db->query("SELECT `userid`, `username`, `level`, `money`, `crystals`
                     FROM `users`
