...
function new_shop_submit()
{
	...
	if (empty($_POST['sn']) || empty($_POST['sd'])) {
		...
		echo 'The ' . $_POST['sn'] . ' Shop was successfully added to the game.<br />
...
function new_stock_submit()
{
	...
	$q  = $db->query('SELECT COUNT(`shopID`)
                     FROM `shops`
                     WHERE `shopID` = ' . $_POST['shop']);
	$q2 = $db->query('SELECT COUNT(`itmid`)
                     FROM `items`
	...
	echo 'Item ID ' . $_POST['item'] . ' was successfully added to shop ID ' . $_POST['shop'] . '<br />
            &gt; <a href="staff.php">Go Home</a>';
	die($h->endpage());
}
function delshop()
