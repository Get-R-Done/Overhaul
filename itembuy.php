...
if (empty($_GET['ID']) OR empty($_POST['qty'])) {
	...
	if ($db->num_rows($q) == 0) {
		...
		$db->query("INSERT INTO `itembuylogs`
                 VALUES (NULL, $userid, {$itemd['itmid']}, $price, {$_POST['qty']},
                  " . time() . ", '{$ib_log}')");
		echo 'You bought ' . $_POST['qty'] . ' ' . $itemd['itmname'] . ' ' . (($_POST['qty'] > 1) ? 's' : '') . ' for ' . money_formatter($price) . '<br />&gt; <a href="inventory.php">Goto your inventory</a>';
	}
	$db->free_result($q);
}
$h->endpage();
