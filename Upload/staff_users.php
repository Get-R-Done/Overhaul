...
function new_user_submit()
{
	...
	$_POST['email']      = (isset($_POST['email']) && filter_input(INPUT_POST, 'email', FILTER_VALIDATE_EMAIL)) ? $db->escape(stripslashes($_POST['email'])) : '';
	$ulevel              = (isset($_POST['user_level']) && in_array($_POST['user_level'], array('1','0'), true)) ? $_POST['user_level'] : FALSE;
	$level               = (isset($_POST['level']) && is_numeric($_POST['level'])) ? abs(intval($_POST['level'])) : 1;
	$money               = (isset($_POST['money']) && is_numeric($_POST['money'])) ? abs(intval($_POST['money'])) : 100;
	$crystals            = (isset($_POST['crystals']) && is_numeric($_POST['crystals'])) ? abs(intval($_POST['crystals'])) : 0;
	$donator             = (isset($_POST['donatordays']) && is_numeric($_POST['donatordays'])) ? abs(intval($_POST['donatordays'])) : 0;
	$_POST['gender']     = (isset($_POST['gender']) && in_array($_POST['gender'], array('Male','Female'), true)) ? $_POST['gender'] : 'Male';
	$strength            = (isset($_POST['strength']) && is_numeric($_POST['strength'])) ? abs(intval($_POST['strength'])) : 10;
	$agility             = (isset($_POST['agility']) && is_numeric($_POST['agility'])) ? abs(intval($_POST['agility'])) : 10;
	$guard               = (isset($_POST['guard']) && is_numeric($_POST['guard'])) ? abs(intval($_POST['guard'])) : 10;
	$labour              = (isset($_POST['labour']) && is_numeric($_POST['labour'])) ? abs(intval($_POST['labour'])) : 10;
	$iq                  = (isset($_POST['iq']) && is_numeric($_POST['iq'])) ? abs(intval($_POST['iq'])) : 10;
	$_POST['username']   = (isset($_POST['username']) && preg_match("/^[a-z0-9_]+([\\s]{1}[a-z0-9_]|[a-z0-9_])+$/i", $_POST['username']) && ((strlen($_POST['username']) < 32) && (strlen($_POST['username']) >= 3))) ? $db->escape(strip_tags(stripslashes($_POST['username']))) : '';
	$_POST['login_name'] = (isset($_POST['login_name']) && preg_match("/^[a-z0-9_]+([\\s]{1}[a-z0-9_]|[a-z0-9_])+$/i", $_POST['login_name']) && ((strlen($_POST['login_name']) < 32) && (strlen($_POST['login_name']) >= 3))) ? $db->escape(strip_tags(stripslashes($_POST['login_name']))) : '';
	$_POST['userpass']   = (isset($_POST['userpass']) && (strlen(stripslashes($_POST['userpass'])) <= 32)) ? stripslashes($_POST['userpass']) : '';
	if (empty($_POST['username']) || empty($_POST['login_name']) || empty($_POST['userpass']) || is_bool($ulevel) || empty($_POST['email']) || empty($level)) {
		echo '
	...
	$ucnt = $db->query('SELECT COUNT(`userid`)
                     FROM `users`
...
function edit_user_form()
{
	...
	$d = $db->query("SELECT `hospreason`, `jail_reason`, `username`,
	...
	$db->free_result($d);
	$itemi['hospreason']  = htmlentities($itemi['hospreason'], ENT_QUOTES, 'ISO-8859-1');
	$itemi['jail_reason'] = htmlentities($itemi['jail_reason'], ENT_QUOTES, 'ISO-8859-1');
	$itemi['username']    = htmlentities($itemi['username'], ENT_QUOTES, 'ISO-8859-1');
	$itemi['login_name']  = htmlentities($itemi['login_name'], ENT_QUOTES, 'ISO-8859-1');
	$itemi['duties']      = htmlentities($itemi['duties'], ENT_QUOTES, 'ISO-8859-1');
	$itemi['staffnotes']  = htmlentities($itemi['staffnotes'], ENT_QUOTES, 'ISO-8859-1');
	$itemi['mb_reason']   = htmlentities($itemi['mb_reason'], ENT_QUOTES, 'ISO-8859-1');
	$itemi['fb_reason']   = htmlentities($itemi['fb_reason'], ENT_QUOTES, 'ISO-8859-1');
...
function edit_user_sub()
{
	...
	$_POST['username']    = (isset($_POST['username']) && preg_match("/^[a-z0-9_]+([\\s]{1}[a-z0-9_]|[a-z0-9_])+$/i", $_POST['username']) && ((strlen($_POST['username']) < 32) && (strlen($_POST['username']) >= 3))) ? $db->escape(strip_tags(stripslashes($_POST['username']))) : '';
	$_POST['login_name']  = (isset($_POST['login_name']) && preg_match("/^[a-z0-9_]+([\\s]{1}[a-z0-9_]|[a-z0-9_])+$/i", $_POST['login_name']) && ((strlen($_POST['login_name']) < 32) && (strlen($_POST['login_name']) >= 3))) ? $db->escape(strip_tags(stripslashes($_POST['login_name']))) : '';
	$_POST['duties']      = (isset($_POST['duties']) && (strlen($_POST['duties']) <= 500)) ? $db->escape(strip_tags(stripslashes($_POST['duties']))) : '';
	$_POST['staffnotes']  = (isset($_POST['staffnotes']) && (strlen($_POST['staffnotes']) <= 500)) ? $db->escape(strip_tags(stripslashes($_POST['staffnotes']))) : '';
	$_POST['level']       = (isset($_POST['level']) && is_numeric($_POST['level'])) ? abs(intval($_POST['level'])) : 1;
	$_POST['money']       = (isset($_POST['money']) && is_numeric($_POST['money'])) ? abs(intval($_POST['money'])) : 100;
	$_POST['bankmoney']   = (isset($_POST['bankmoney']) && is_numeric($_POST['bankmoney'])) ? abs(intval($_POST['bankmoney'])) : 0;
	$_POST['cybermoney']  = (isset($_POST['cybermoney']) && is_numeric($_POST['cybermoney'])) ? abs(intval($_POST['cybermoney'])) : 0;
	$_POST['crystals']    = (isset($_POST['crystals']) && is_numeric($_POST['crystals'])) ? abs(intval($_POST['crystals'])) : 0;
	$_POST['mailban']     = (isset($_POST['mailban']) && is_numeric($_POST['mailban'])) ? abs(intval($_POST['mailban'])) : 0;
	$_POST['mb_reason']   = (isset($_POST['mb_reason']) && (strlen($_POST['mb_reason']) <= 500)) ? $db->escape(strip_tags(stripslashes($_POST['mb_reason']))) : '';
	$_POST['forumban']    = (isset($_POST['forumban']) && is_numeric($_POST['forumban'])) ? abs(intval($_POST['forumban'])) : 0;
	$_POST['fb_reason']   = (isset($_POST['fb_reason']) && (strlen($_POST['fb_reason']) <= 500)) ? $db->escape(strip_tags(stripslashes($_POST['fb_reason']))) : '';
	$_POST['hospital']    = (isset($_POST['hospital']) && is_numeric($_POST['hospital'])) ? abs(intval($_POST['hospital'])) : 0;
	$_POST['hospreason']  = (isset($_POST['hospreason']) && (strlen($_POST['hospreason']) <= 500)) ? $db->escape(strip_tags(stripslashes($_POST['hospreason']))) : '';
	$_POST['jail']        = (isset($_POST['jail']) && is_numeric($_POST['jail'])) ? abs(intval($_POST['jail'])) : 0;
	$_POST['jail_reason'] = (isset($_POST['jail_reason']) && (strlen($_POST['jail_reason']) <= 500)) ? $db->escape(strip_tags(stripslashes($_POST['jail_reason']))) : '';
	$maxwill              = (isset($_POST['maxwill']) && is_numeric($_POST['maxwill'])) ? abs(intval($_POST['maxwill'])) : 1;
	$_POST['strength']    = (isset($_POST['strength']) && is_numeric($_POST['strength'])) ? abs(intval($_POST['strength'])) : 10;
	$_POST['agility']     = (isset($_POST['agility']) && is_numeric($_POST['agility'])) ? abs(intval($_POST['agility'])) : 10;
	$_POST['guard']       = (isset($_POST['guard']) && is_numeric($_POST['guard'])) ? abs(intval($_POST['guard'])) : 10;
	$_POST['labour']      = (isset($_POST['labour']) && is_numeric($_POST['labour'])) ? abs(intval($_POST['labour'])) : 10;
	$_POST['IQ']          = (isset($_POST['IQ']) && is_numeric($_POST['IQ'])) ? abs(intval($_POST['IQ'])) : 10;
	if (empty($_POST['username']) || empty($_POST['login_name']) || empty($_POST['userid']) || empty($maxwill) || empty($_POST['level'])) {
		echo '
        You missed one or more of the required fields, Please go back and try again.
        <br />
        &gt; <a href="staff_users.php?action=edituser">Go Back</a>
	...
	$h_exists = $db->query('SELECT COUNT(`hID`)
                     FROM `houses`
                     WHERE `hWILL` = ' . $maxwill);
	...
	$db->free_result($h_exists);
	$u = $db->query("SELECT COUNT(`userid`)
                     FROM `users`
                     WHERE `username` = '{$_POST['username']}'
...
function deluser()
{
	...
	switch ($_GET['step']) {
		...
		case 2:
			$_POST['user'] = (isset($_POST['user']) && is_numeric($_POST['user'])) ? abs(intval($_POST['user'])) : 0;
			staff_csrf_stdverify('staff_deluser1', 'staff_users.php?action=deluser');
        ...
        <form action='staff_users.php?action=deluser&amp;step=3' method='post'>
           ...
           ";
			break;
		case 3:
			staff_csrf_stdverify('staff_deluser2', 'staff_users.php?action=deluser');
			$_POST['userid']  = (isset($_POST['userid']) && is_numeric($_POST['userid'])) ? abs(intval($_POST['userid'])) : 0;
			...
			$db->query('DELETE FROM `users`
         		 WHERE `userid` = ' . $_POST['userid']);
			$db->query('DELETE FROM `userstats`
                 WHERE `userid` = ' . $_POST['userid']);
			$db->query('DELETE FROM `inventory`
                 WHERE `inv_userid` = ' . $_POST['userid']);
			$db->query('DELETE FROM `fedjail`
                 WHERE `fed_userid` = ' . $_POST['userid']);
			stafflog_add('Deleted User ' . $username . ' [' . $_POST['userid'] . ']');
...
function inv_user_view()
{
	...
	$inv = $db->query('SELECT `inv_qty`, `inv_id`, `itmname`, `itmsellprice`
	...
	if ($db->num_rows($inv) == 0) {
		...
		while ($i = $db->fetch_row($inv)) {
			echo '
			<tr>
            	<td>' . $i['itmname'] . ' ' . (($i['inv_qty'] > 1) ? '&nbsp;x' . $i['inv_qty'] : '') . '</td>
            	<td>' . money_formatter($i['itmsellprice']) . '</td>
            	<td>' . money_formatter($i['itmsellprice'] * $i['inv_qty']) . '</td>
            	<td>
...
function inv_delete()
{
	...
	$d = $db->query('SELECT COUNT(`inv_id`)
                     FROM `inventory`
...
function credit_user_submit()
{
	...
	staff_csrf_stdverify('staff_credituser', 'staff_users.php?action=creditform');
	$_POST['user']     = (isset($_POST['user']) && is_numeric($_POST['user'])) ? abs(intval($_POST['user'])) : 0;
	$_POST['money']    = (isset($_POST['money']) && is_numeric($_POST['money'])) ? abs(intval($_POST['money'])) : 0;
	$_POST['crystals'] = (isset($_POST['crystals']) && is_numeric($_POST['crystals'])) ? abs(intval($_POST['crystals'])) : 0;
	if ((empty($_POST['money']) && empty($_POST['crystals'])) || empty($_POST['user'])) {
		echo '
	...
	$db->query("UPDATE `users`
             ...
             WHERE `userid` = {$_POST['user']}");
	$un = htmlentities($db->fetch_single($d), ENT_QUOTES, 'ISO-8859-1');
	stafflog_add('Credited ' . $un . ' [' . $_POST['user'] . '] ' . money_formatter($_POST['money']) . ' and/or ' . number_format($_POST['crystals']) . ' crystals.');
	echo $un . ' [' . $_POST['user'] . '] was credited with ' . money_formatter($_POST['money']) . ' and/or ' . number_format($_POST['crystals']) . ' crystals.
	<br />
	&gt; <a href="staff.php">Go Back</a>
   	';
}
...
function mcredit_user_submit()
{
	...
	staff_csrf_stdverify('staff_masscredit', 'staff_users.php?action=masscredit');
	$_POST['money']    = (isset($_POST['money']) && is_numeric($_POST['money'])) ? abs(intval($_POST['money'])) : 0;
	$_POST['crystals'] = (isset($_POST['crystals']) && is_numeric($_POST['crystals'])) ? abs(intval($_POST['crystals'])) : 0;
	...
	$db->query("UPDATE `users`
             SET `money` = `money` + {$_POST['money']},
             `crystals` = `crystals` + {$_POST['crystals']}");
	stafflog_add('Credited all users ' . money_formatter($_POST['money']) . ' and/or ' . number_format($_POST['crystals']) . ' crystals.');
	echo "
	All Users credited.
...
function reports_view()
{
	...
	$q    = $db->query("SELECT `prID`, `prTEXT`, `prREPORTED`, `prREPORTER`,
	...
	while ($r = $db->fetch_row($q)) {
		...
		<tr>
			...
			<td>" . htmlentities($r['prTEXT'], ENT_QUOTES, 'ISO-8859-1') . "</td>
			<td>
...
function forcelogout()
{
	...
	if (!empty($_POST['userid'])) {
		staff_csrf_stdverify('staff_forcelogout', 'staff_users.php?action=forcelogout');
		$d = $db->query('SELECT COUNT(`userid`)
                         FROM `users`
                         WHERE `userid` = ' . $_POST['userid']);
		if ($db->fetch_single($d) == 0) {
			$db->free_result($d);
		...
		echo '
        User ID ' . $_POST['userid'] . ' successfully forced to logout.
        <br />
        &gt; <a href="staff.php">Go Home</a>
           ';
	} else {
...
function report_clear()
{
	...
	staff_csrf_stdverify('staff_clear_preport', 'staff_users.php?action=reportsview');
	$_POST['ID'] = (isset($_POST['ID']) && is_numeric($_POST['ID'])) ? abs(intval($_POST['ID'])) : 0;
	if (empty($_POST['ID'])) {
		echo '
