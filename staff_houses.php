...
function addhouse()
{
	...
	if ($price && $will && $name) {
		staff_csrf_stdverify('staff_addhouse', 'staff_houses.php?action=addhouse');
		$q = $db->query("SELECT COUNT(`hID`)
                         FROM `houses`
                         WHERE `hWILL` = {$will}");
		...
		$db->free_result($q);
		$db->query("INSERT INTO `houses`
                 VALUES(NULL, '$name', '$price', '$will')");
		stafflog_add('Created House ' . $name);
		echo 'House ' . $name . ' added to the game.<br />
                &gt; <a href="staff.php">Go Back</a>';
		die($h->endpage());
	} else {
...
function edithouse()
{
	...
	switch ($_POST['step']) {
		case "2":
			$price       = (isset($_POST['price']) && is_numeric($_POST['price'])) ? abs(intval($_POST['price'])) : 0;
			$will        = (isset($_POST['will']) && is_numeric($_POST['will'])) ? abs(intval($_POST['will'])) : 0;
			...
			staff_csrf_stdverify('staff_edithouse2', 'staff_houses.php?action=edithouse');
			$q = $db->query("SELECT `hID`
                         FROM `houses`
			...
			echo 'House ' . $name . ' was edited successfully.<br />
                &gt; <a href="staff_houses.php?action=edithouse">Go Back</a>';
			die($h->endpage());
			break;
		case "1":
			$_POST['house'] = (isset($_POST['house']) && is_numeric($_POST['house'])) ? abs(intval($_POST['house'])) : 0;
			staff_csrf_stdverify('staff_edithouse1', 'staff_houses.php?action=edithouse');
			$q = $db->query("SELECT `hWILL`, `hPRICE`, `hNAME`
                         FROM `houses`
...
{
	global $db, $ir, $c, $h, $userid;
	$_POST['house'] = (isset($_POST['house']) && is_numeric($_POST['house'])) ? abs(intval($_POST['house'])) : '';
	if ($_POST['house']) {
		staff_csrf_stdverify('staff_delhouse', 'staff_houses.php?action=delhouse');
		$q = $db->query("SELECT `hWILL`, `hPRICE`, `hID`, `hNAME`
                         FROM `houses`
                         WHERE `hID` = {$_POST['house']}");
		if ($db->num_rows($q) == 0) {
			$db->free_result($q);
		...
		$db->query("UPDATE `users`
                 ...
                 WHERE `maxwill` = {$old['hWILL']}");
		$db->query("DELETE FROM `houses`
         		 WHERE `hID` = {$old['hID']}");
		stafflog_add('Deleted house ' . $old['hNAME']);
		echo 'House ' . $old['hNAME'] . ' deleted.<br />
                &gt; <a href="staff_houses.php?action=delhouse">Go Back</a>';
		die($h->endpage());
	} else {
