...
<a href='bank.php'>Start using my account</a>";
			$db->query("UPDATE `users` SET `money` = `money` - {$bank_cost}, `bankmoney` = 0 WHERE `userid` = $userid");
		} else {
...
function index()
{
	...
	echo "\n<b>You currently have" . money_formatter($ir['bankmoney']) . " in the bank.</b><br />
...
It will cost you {$bank_feepercent}% of the money you deposit, rounded up. The maximum fee is " . money_formatter($bank_maxfee) . ".<form action='bank.php?action=deposit' method='post'>
...
function deposit()
{
	...
	if ($_POST['deposit'] > $ir['money']) {
		...
		$ir['bankmoney'] += $gain;
		$db->query("UPDATE `users` SET `bankmoney` = `bankmoney` + $gain,
